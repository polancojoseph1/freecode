...
export default new Hono<{ Bindings: Env }>()
  ...
  .post("/feishu", async (c) => {
    // 🛡️ Sentinel: Added missing webhook signature validation to prevent spoofing
    ...
    if (!timestamp || !nonce || !signature) {
      return c.json({ error: "Missing required signature headers" }, { status: 401 })
    }
    ...
    const computedHashBuf = Buffer.from(computedHash)
    const signatureBuf = Buffer.from(signature)

    if (computedHashBuf.length !== signatureBuf.length || !timingSafeEqual(computedHashBuf, signatureBuf)) {

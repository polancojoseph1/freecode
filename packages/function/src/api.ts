...
export default new Hono<{ Bindings: Env }>()
  ...
  .post("/feishu", async (c) => {
    ...
    if (!timestamp || !nonce || !signature) {
      return c.text("Unauthorized: missing signature headers", { status: 401 })
    }

    const strToHash = timestamp + nonce + Resource.FEISHU_WEBHOOK_SECRET.value + rawBody
    const expectedSignature = createHash("sha256").update(strToHash).digest("hex")
    ...
    if (signatureBuf.length !== expectedSignatureBuf.length || !timingSafeEqual(signatureBuf, expectedSignatureBuf)) {
      return c.text("Unauthorized: invalid signature", { status: 401 })
    }

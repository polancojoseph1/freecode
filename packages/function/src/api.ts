...
export default new Hono<{ Bindings: Env }>()
  ...
  .post("/feishu", async (c) => {
    ...
    const computedSignatureBuf = Buffer.from(computedSignature)

    if (signatureBuf.length !== computedSignatureBuf.length || !timingSafeEqual(signatureBuf, computedSignatureBuf)) {
      return c.text("Unauthorized", { status: 401 })
    }

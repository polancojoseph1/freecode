...
export default new Hono<{ Bindings: Env }>()
  ...
  .post("/feishu", async (c) => {
    ...
    if (!timestamp || !nonce || !signature) {
      return c.json({ error: "Missing webhook signature headers" }, { status: 401 })
    }

    const rawBody = await c.req.text()

    const expectedSignatureStr = createHash("sha256")
      .update(`${timestamp}${nonce}${Resource.FEISHU_WEBHOOK_SECRET.value}${rawBody}`)
      .digest("hex")

    const expectedSigBuffer = Buffer.from(expectedSignatureStr)

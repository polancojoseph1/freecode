...
export default new Hono<{ Bindings: Env }>()
  ...
  .post("/feishu", async (c) => {
    ...
    const expectedSignatureBuffer = Buffer.from(hash)
    const incomingSignatureBuffer = Buffer.from(signature)

    if (
      expectedSignatureBuffer.length !== incomingSignatureBuffer.length ||
      !timingSafeEqual(expectedSignatureBuffer, incomingSignatureBuffer)
    ) {

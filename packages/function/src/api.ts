...
export default new Hono<{ Bindings: Env }>()
  ...
  .post("/feishu", async (c) => {
    ...
    const computedSignature = createHash("sha256").update(strToHash).digest("base64");

    // Convert to buffers to avoid timing attacks, checking length first to prevent exceptions

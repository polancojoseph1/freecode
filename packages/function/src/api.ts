...
export default new Hono<{ Bindings: Env }>()
  ...
  .post("/feishu", async (c) => {
    ...
    const computedSig = Buffer.from(hash)
    const expectedSig = Buffer.from(signature)

    if (computedSig.length !== expectedSig.length || !timingSafeEqual(computedSig, expectedSig)) {

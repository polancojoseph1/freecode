  ...
  builder: (yargs: Argv) => {
    return yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to FREECODE_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
      ...
      .option("steps", {
        type: "number",
        describe: "maximum agentic steps per turn (default: unlimited)",
      })
  },
  handler: async (args) => {
    if (args.steps !== undefined) {
      process.env["FREECODE_MAX_STEPS"] = String(args.steps)
    }
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.FREECODE_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: OpencodeClient) {
      function tool(part: ToolPart) {
        try {
          if (part.tool === "bash") return shell(props<typeof ShellTool>(part))

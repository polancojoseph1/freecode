...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g freecode-ai",
      pnpm: "pnpm uninstall -g freecode-ai",
      bun: "bun remove -g freecode-ai",
      yarn: "yarn global remove freecode-ai",
      brew: "brew uninstall freecode",
      choco: "choco uninstall freecode",
      scoop: "scoop uninstall freecode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "freecode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# freecode") || content.includes(".freecode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".freecode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".freecode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".freecode"))
    ) {
      continue
    }

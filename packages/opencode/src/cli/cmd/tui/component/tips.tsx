...
const TIPS = [
  ...
  "Create {highlight}freecode.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/freecode/tui.json{/highlight} for global config",
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.freecode/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.freecode/agent/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.freecode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.freecode/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent OpenCode from reading sensitive files",
  "Use {highlight}freecode run{/highlight} for non-interactive scripting",
  "Use {highlight}freecode --continue{/highlight} to resume the last session",
  "Use {highlight}freecode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}freecode serve{/highlight} for headless API access to FreeCode",
  "Use {highlight}freecode run --attach{/highlight} to connect to a running server",
  "Run {highlight}freecode upgrade{/highlight} to update to the latest version",
  "Run {highlight}freecode auth list{/highlight} to see all configured providers",
  "Run {highlight}freecode agent create{/highlight} for guided agent creation",
  "Use {highlight}/freecode{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}freecode github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/freecode fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.freecode/themes/{/highlight} directory",
  ...
  "Run {highlight}freecode debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/freecode{/highlight} for containerized use",

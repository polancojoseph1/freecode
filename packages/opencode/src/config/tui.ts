...
export namespace TuiConfig {
  ...
  const state = Instance.state(async () => {
    ...
    // ⚡ Bolt Optimization:
    // We hoist the instantiation of all config file promises and wrap them
    // in a single `Promise.all` to execute the file I/O operations concurrently,
    // avoiding N+1 synchronous delays. We then apply the merges sequentially
    // to preserve the strict precedence order.
    const [
      globalInfos,
      customInfo,
      projectInfos,

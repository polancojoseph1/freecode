...
function globalFiles() {
  ...
  if (Flag.FREECODE_CONFIG_DIR) {
    files.push(path.join(Flag.FREECODE_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.FREECODE_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.FREECODE_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no FREECODE_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.FREECODE_CONFIG_DIR, Flag.FREECODE_CONFIG_DIR).catch(() => [])
}

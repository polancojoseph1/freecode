...
const managedConfigDir = process.env.FREECODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "freecode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "freecode.json") {
...
test("ignores legacy tui keys in freecode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://opencode.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "freecode.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["FREECODE_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      freecode: {
        options: {
          apiKey: "{env:FREECODE_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["freecode"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
    ...
    if (originalControlToken !== undefined) {
      process.env["FREECODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["FREECODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "freecode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .freecode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const freecodeDir = path.join(dir, ".freecode")
      await fs.mkdir(freecodeDir, { recursive: true })
      const agentDir = path.join(freecodeDir, "agent")
...
test("loads agents from .freecode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const freecodeDir = path.join(dir, ".freecode")
      await fs.mkdir(freecodeDir, { recursive: true })

      const agentsDir = path.join(freecodeDir, "agents")
...
test("loads commands from .freecode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const freecodeDir = path.join(dir, ".freecode")
      await fs.mkdir(freecodeDir, { recursive: true })

      const commandDir = path.join(freecodeDir, "command")
...
test("loads commands from .freecode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const freecodeDir = path.join(dir, ".freecode")
      await fs.mkdir(freecodeDir, { recursive: true })

      const commandsDir = path.join(freecodeDir, "commands")
...
test("does not try to install dependencies in read-only FREECODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.FREECODE_CONFIG_DIR
  process.env.FREECODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.FREECODE_CONFIG_DIR
    else process.env.FREECODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable FREECODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "freecode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .freecode config
      const projectDir = path.join(dir, "project")
      const freecodeDir = path.join(projectDir, ".freecode")
      await fs.mkdir(freecodeDir, { recursive: true })
      ...
      // Local .freecode config with different plugins
      await Filesystem.write(
        path.join(freecodeDir, "freecode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .freecode config with some overlapping plugins
      await Filesystem.write(
        path.join(freecodeDir, "freecode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets FREECODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .freecode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .freecode directory config enables it
      const freecodeDir = path.join(dir, ".freecode")
      await fs.mkdir(freecodeDir, { recursive: true })
      await Filesystem.write(
        path.join(freecodeDir, "freecode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/freecode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/freecode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-freecode@2.4.3")).toBe("oh-my-freecode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@freecode/plugin@2.0.0")).toBe("@freecode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-freecode@2.4.3", "file:///project/.freecode/plugin/oh-my-freecode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.freecode/plugin/oh-my-freecode.js")
  })
  ...
  test("local plugin directory overrides global freecode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const freecodeDir = path.join(projectDir, ".freecode")
        const pluginDir = path.join(freecodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("FREECODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["FREECODE_DISABLE_PROJECT_CONFIG"]
    process.env["FREECODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["FREECODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["FREECODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .freecode/ directories when flag is set", async () => {
    const originalEnv = process.env["FREECODE_DISABLE_PROJECT_CONFIG"]
    process.env["FREECODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .freecode directory with a command
          const freecodeDir = path.join(dir, ".freecode", "command")
          await fs.mkdir(freecodeDir, { recursive: true })
          await Filesystem.write(path.join(freecodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .freecode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["FREECODE_DISABLE_PROJECT_CONFIG"]
    ...
    try {
      ...
      if (originalDisable === undefined) {
        ...
        process.env["FREECODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
  ...
  test("FREECODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["FREECODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["FREECODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["FREECODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["FREECODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from FREECODE_CONFIG_DIR, not project
...
describe("FREECODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in FREECODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["FREECODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["FREECODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["FREECODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["FREECODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in FREECODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["FREECODE_CONFIG_CONTENT"]

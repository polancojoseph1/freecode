...
test("discovers skills from .freecode/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".freecode", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".freecode", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
...
`,
  ...
  const home = process.env.FREECODE_TEST_HOME
  process.env.FREECODE_TEST_HOME = tmp.path
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".freecode", "skill", "dir-skill")
...
test("discovers multiple skills from .freecode/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".freecode", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".freecode", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".freecode", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.FREECODE_TEST_HOME
  process.env.FREECODE_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.FREECODE_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const freecodeSkillDir = path.join(dir, ".freecode", "skill", "agent-skill")
      const freecodeSkillsDir = path.join(dir, ".freecode", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(freecodeSkillDir, "SKILL.md"),
        `---
name: freecode-skill
description: A skill in the .freecode/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(freecodeSkillsDir, "SKILL.md"),
        `---
name: freecode-skill
description: A skill in the .freecode/skills directory.
---

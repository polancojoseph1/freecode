...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, freecodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.freecode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.freecode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              freecode: { itemId: "msg_freecode_part" },
    ...
    expect(result[0].providerOptions?.freecode?.itemId).toBe("msg_freecode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.freecode?.itemId).toBe("msg_freecode_part")

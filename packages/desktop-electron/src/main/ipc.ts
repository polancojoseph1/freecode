...
export function registerIpcHandlers(deps: Deps) {
  ...
  ipcMain.handle("open-path", async (_event: IpcMainInvokeEvent, path: string, app?: string) => {
    ...
    const exists = await Promise.resolve(deps.checkAppExists(app))
    if (!exists) {
      throw new Error(`Application ${app} does not exist.`)
    }

    const resolvedApp = process.platform === "win32" ? await Promise.resolve(deps.resolveAppPath(app)) : app
    if (!resolvedApp) {
    ...
    await new Promise<void>((resolve, reject) => {
      const [cmd, args] =
        process.platform === "darwin"
          ? (["open", ["-a", resolvedApp, path]] as const)
          : ([resolvedApp, [path]] as const)

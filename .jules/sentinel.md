...
**Vulnerability:** The `/feishu` webhook endpoint previously accepted incoming HTTP requests without verifying the payload's origin or integrity, exposing the system to potential spoofing attacks where arbitrary messages could be sent to the Discord support channel.
**Learning:** External webhook providers typically offer a request signature mechanism to cryptographically guarantee that incoming events are authentic. In Cloudflare Workers/SST setups, it's essential to correctly parse raw request bodies to avoid stream exhaustion while computing these hashes.
**Prevention:** Always implement signature validation (e.g., extracting timestamps, nonces, and signatures, then computing a SHA-256 hash or HMAC) on public-facing webhook receivers. Use `timingSafeEqual` to avoid timing side channels, and ensure buffer lengths are explicitly checked prior to constant-time comparison.

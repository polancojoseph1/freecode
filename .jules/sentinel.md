...
## 2025-06-20 - Missing Webhook Authentication
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` was entirely missing signature verification and authentication, allowing any unauthenticated attacker to post arbitrary payloads and trigger internal bot actions.
**Learning:** Webhook endpoints must explicitly validate the incoming signature against a configured secret. Allowing requests to process payloads without checking the required headers (e.g. `x-lark-signature`) creates a critical authentication bypass.
**Prevention:** Always implement signature validation early in the handler. Reject requests with missing headers immediately (e.g. 401 Unauthorized), verify the hash (using `timingSafeEqual`), and avoid consuming the request body stream (`c.req.json()`) before reading the raw body (`c.req.text()`) needed for hashing.

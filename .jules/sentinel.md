...
## 2024-07-18 - Missing Webhook Signature Validation for Feishu Endpoint
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` was processing payloads directly using `c.req.json()` without verifying the request origin, leaving it susceptible to unauthenticated payloads.
**Learning:** External webhook endpoints often omit signature validation which allows anyone to send arbitrary payloads. The Feishu signature validation requires reading `x-lark-request-timestamp`, `x-lark-request-nonce`, and `x-lark-signature`, generating a sha256 hash (not HMAC), and converting to hex format. `rawBody` stream extraction needs `c.req.text()` since reading `.json()` first corrupts stream reading.
**Prevention:** Always mandate webhook signature validation checks explicitly in external-facing endpoints. Reject any requests lacking validation headers immediately before performing any JSON body parsing or stream consumption. Compare buffered hex signatures explicitly using constant-time algorithms (`timingSafeEqual`) and verify buffer byte-length equality to avoid timing attacks and bypass errors.

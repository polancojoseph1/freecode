...
## 2025-05-25 - Missing Feishu Webhook Security Verification
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` lacked signature verification, making it vulnerable to unauthorized payload forging and Server-Side Request Forgery (SSRF) attacks. An attacker could send crafted requests and trigger Discord bot messages or other downstream actions.
**Learning:** Feishu webhooks use a specific signature generation mechanism involving a SHA-256 hash (not HMAC) of `timestamp + nonce + secret + body`. Furthermore, extracting the raw body string (`await c.req.text()`) is necessary for precise signature validation, and any missing signature headers must result in an immediate rejection (early return) to prevent authentication bypass.
**Prevention:** Always implement signature verification for webhooks. Explicitly check for all required headers (timestamp, nonce, signature), use `await c.req.text()` to get the exact raw body for hashing (parsing it manually afterward with `JSON.parse()`), use a cryptographically secure comparison (e.g., `timingSafeEqual`), and explicitly verify the byte lengths of buffers before comparison to prevent errors. Ensure the secret is securely stored and retrieved (e.g., via SST Resources).

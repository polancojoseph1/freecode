...
## 2025-06-03 - Insecure Randomness Fallback in Core ID Generation
**Vulnerability:** Core identifier generation functions (`uuid.ts` and `id.ts`) silently fell back to using `Math.random()` to generate IDs/UUIDs if the Web Crypto API (`crypto.getRandomValues()` or `crypto.randomUUID()`) was unavailable or threw an error (e.g., in non-secure contexts). This leads to highly predictable identifiers.
**Learning:** Fallback mechanisms designed for "robustness" (ensuring an ID is always generated) inadvertently bypassed core security guarantees. When cryptographic entropy is unavailable, a system should "fail closed" rather than silently degrading security. The tests were also explicitly asserting for this insecure fallback behavior.
**Prevention:** Remove fallback logic that weakens cryptographic boundaries. If secure randomness is not available, explicitly throw an Error to fail loudly and securely. Never use `Math.random()` for identifier or session key generation, even as a fallback. Tests should be updated to expect the error state rather than asserting on weak pseudo-random strings.

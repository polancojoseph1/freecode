...
## $(date +%Y-%m-%d) - Feishu Webhook Unverified Signature
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` processed incoming requests without validating the `x-lark-signature` header, allowing attackers to forge webhook requests and impersonate the Feishu integration.
**Learning:** Webhook endpoints must securely verify incoming payloads using the exact cryptographic algorithm specified by the provider (e.g., concatenating timestamp, nonce, secret, and the raw body for SHA256). Missing validation or incorrect body reading (like using parsed JSON instead of raw text) invalidates security.
**Prevention:** Always fail-closed on missing signature headers, compute hashes using the exact provider format with `createHash`, and compare signatures using constant-time algorithms (`timingSafeEqual`) on byte buffers. Ensure `await c.req.text()` is used to get the raw body before parsing JSON.

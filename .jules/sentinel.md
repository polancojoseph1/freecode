...
## 2025-03-24 - Missing Webhook Authentication on Feishu Endpoint
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` lacked authentication or signature verification. It blindly parsed JSON payloads allowing any external actor to submit forged webhook events (e.g., messages via Discord bot integrations) without validation.
**Learning:** External webhook endpoints receiving payloads often rely on cryptographic signatures in headers for authentication. Failing to validate these signatures leaves internal resources (like posting to Discord channels via the bot token) exposed to unauthenticated abuse. The `x-lark-signature` must be computed precisely by hashing `timestamp + nonce + WEBHOOK_SECRET + rawBody` with SHA-256 and encoding to hex, using standard validation tools like `timingSafeEqual`.
**Prevention:** Always ensure that externally facing webhook APIs implement robust authentication checks. When parsing webhook bodies for signature verification, extract the `rawBody` as text first (e.g., `await c.req.text()`) and explicitly compare buffer lengths before invoking `timingSafeEqual` to avoid bypassing via multi-byte characters or throwing exceptions. Reject requests early with a 401 status if the required headers are missing, preventing unauthenticated processing.

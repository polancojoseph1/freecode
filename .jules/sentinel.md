...
## 2024-05-15 - [CRITICAL] Fix Feishu webhook signature validation bypass
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` processed payloads blindly without verifying the authenticity of incoming requests using the signature header.
**Learning:** External webhooks from providers like Feishu/Lark often include standard security headers (`x-lark-request-timestamp`, `x-lark-request-nonce`, `x-lark-signature`) that must be validated to prevent unauthorized actors from spoofing payloads (which in this case triggers a bot message via Discord).
**Prevention:** Always implement signature validation for webhook endpoints. Extract raw body with `c.req.text()`, securely compute the required hash format (e.g. SHA-256 for Feishu), and compare using `timingSafeEqual` with strict length checks to mitigate both spoofing and timing attacks. Ensure the validation correctly returns 401 early if headers are missing.

...
## 2025-09-09 - Feishu Webhook Security Vulnerability
**Vulnerability:** The Feishu webhook endpoint at `/feishu` in `packages/function/src/api.ts` lacked signature verification, making it vulnerable to spoofing and replay attacks.
**Learning:** Webhook endpoints must always verify the integrity and origin of incoming payloads using cryptographic signatures provided by the sender. In Cloudflare Workers, `crypto.subtle` is preferred, but `node:crypto` and `node:buffer` can be used via compatibility flags for constant-time comparisons (`timingSafeEqual`) if needed. The signature calculation relies on `x-lark-request-timestamp`, `x-lark-request-nonce`, `FEISHU_WEBHOOK_SECRET`, and the raw request body.
**Prevention:** Always implement signature validation on public webhook endpoints. Consume the raw body first for hashing, perform constant-time comparison on the signature, and only then parse the payload.
## $(date +%Y-%m-%d) - CI Flakiness in Plugin Loading
**Vulnerability:** The test `plugin.auth-override > user plugin overrides built-in github-copilot auth` failed sporadically due to a strict 30-second timeout while simulating plugin installation and loading, triggering runner shutdown signals in GitHub Actions.
**Learning:** Test timeouts that assume fast hardware or network stability often cause flaky CI jobs, which masquerade as runner termination errors. GitHub Actions MacOS/Windows or heavily-loaded runners can take longer than local environments.
**Prevention:** Always allocate generous timeouts for tests involving filesystem operations, dependency resolution, or subprocesses, to decouple test logic validation from infrastructure latency.

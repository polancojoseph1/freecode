...
## 2025-03-05 - Missing webhook signature validation on /feishu endpoint
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` processed incoming payloads without verifying the sender's cryptographic signature, making it vulnerable to forged requests from unauthorized sources.
**Learning:** External webhook providers (like Feishu/Lark) send events over public endpoints. If signatures are not validated, the system assumes trust based purely on the endpoint's obscurity or the payload structure, which can easily be bypassed by attackers.
**Prevention:** Always implement the provider-specific signature verification scheme (in this case, computing a SHA-256 hash using `timestamp + nonce + secret + rawBody`) for all incoming webhooks, and use `timingSafeEqual` with byte-length validation to mitigate timing attacks.

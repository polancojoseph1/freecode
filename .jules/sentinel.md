...
## 2024-05-17 - Arbitrary Code Execution in open-path IPC Handler
**Vulnerability:** The `open-path` IPC handler in `packages/desktop-electron/src/main/ipc.ts` allowed arbitrary applications to be executed via `execFile` because the `app` parameter passed from the renderer was not sufficiently validated or sanitized. It just executed `[cmd, args]` where `cmd` was `open` with `[-a, app, path]` on Darwin, or `app` with `[path]` on other platforms.
**Learning:** In Electron IPC handlers, user input from the renderer process, especially for arguments passed directly to child_process functions like `execFile` or `spawn`, must be treated as untrusted and strictly validated. Relying solely on the presence of an app name or simple checks is insufficient; attackers could pass built-in shell executables or use path traversal to bypass intended execution. We need to validate the existence, resolve the actual executable path securely, and block known dangerous built-in executables like bash, cmd, powershell.
**Prevention:** Always validate executable names or paths from the renderer before passing them to `execFile` or `spawn`. Use blocklists for dangerous built-ins (e.g., `sh`, `bash`, `cmd`, `node`, `powershell`, `wscript`) in conjunction with path traversal checks (`..`) and absolute path resolution using secure dependency injection methods like `deps.checkAppExists` and `deps.resolveAppPath`.

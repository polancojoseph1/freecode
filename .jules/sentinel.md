...
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` was not validating incoming requests with a signature, allowing for unauthenticated access by spoofing.
**Learning:** For endpoints relying on webhooks, they must enforce signature validation on the incoming requests, and missing headers should be rejected early. To make the secret available across SST architectures without running `sst build`, it must be manually added to both `sst-env.d.ts` and the `link` array of the respective infrastructure component.
**Prevention:** Always require signature validation when implementing webhook receivers, explicitly check for missing headers, ensure cryptographic comparison of signatures with `timingSafeEqual`, and verify that `sst-env.d.ts` definitions are updated for new secrets.

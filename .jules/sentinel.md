...
## 2025-02-27 - [Fix command injection vulnerability in app path resolution]
**Vulnerability:** The Electron main process accepted an arbitrary string for the `appName` payload via IPC channels like `open-path`, `check-app-exists`, and `resolve-app-path`. This `appName` string was passed directly to the system `execFile` without validation. If the renderer process was compromised, an attacker could trigger arbitrary app execution.
**Learning:** `execFile` vulnerabilities can manifest not just in standard inputs but also via the IPC interface accepting commands implicitly designed for UI functions like "open IDE" without verifying what that IDE is against an allowlist.
**Prevention:** Only permit known and specific application names to be passed from the renderer processes to system execution paths in Electron IPC handles. Implement an explicit allowlist mapping acceptable IDs to prevent malicious injection.

...
## 2026-04-20 - Command Injection Prevention in Electron Open Path IPC\n**Vulnerability:** Found a command injection/arbitrary execution vulnerability in the Electron `open-path` IPC handler where a malicious renderer could pass any executable path in the `app` parameter, bypassing validation to run arbitrary binaries via `execFile` or `open -a`.\n**Learning:** When resolving or executing application paths provided by the renderer over IPC in Electron, it is unsafe to trust the renderer's string. The renderer process should only pass known application identifiers.\n**Prevention:** Maintain a strict allowlist of permitted applications in the main process. Validate any application identifier received from the renderer against this allowlist before attempting to resolve its absolute path or execute it.

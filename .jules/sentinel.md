...
## $(date +%Y-%m-%d) - Feishu Webhook Signature Validation Missing
**Vulnerability:** The `/feishu` webhook endpoint lacked payload signature validation (`X-Lark-Signature`), allowing unauthorized parties to spoof requests and trigger internal workflows (like sending Discord messages) by sending arbitrary JSON payloads.
**Learning:** Webhook endpoints must default to fail-closed authentication using strong cryptographic verification (e.g., SHA-256 or HMAC) of the raw payload against the signature headers provided by the service provider. Forgetting this validation is a common blind spot when initially prototyping webhook integrations.
**Prevention:** Always implement signature validation on any webhook handler before deploying it. When using Hono or similar frameworks, ensure you compute the signature using the raw body (e.g., `c.req.text()`) rather than the parsed JSON to prevent mismatches caused by whitespace or formatting differences. Ensure missing signatures or lengths trigger an immediate fail-closed response.

...
## 2025-05-25 - Unauthenticated Feishu Webhook Endpoint
**Vulnerability:** The `/feishu` POST webhook endpoint in `packages/function/src/api.ts` accepted incoming events without verifying their authenticity, allowing any unauthenticated external source to spoof Feishu messages and trigger actions (like sending arbitrary support messages to Discord).
**Learning:** Feishu sends events with a verification token that must be matched against a securely stored secret (`FEISHU_VERIFICATION_TOKEN`). Depending on the event type (v1 URL Verification vs v2 standard events), the token might be at the root of the body payload (`body.token`) or nested inside a header object (`body.header.token`).
**Prevention:** Always authenticate webhook endpoints by verifying the provided token or signature against a securely stored secret. When integrating with third-party providers like Feishu, carefully consult their documentation for all payload variations to ensure the verification check handles both v1 and v2 token locations.

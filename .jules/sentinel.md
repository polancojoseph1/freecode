...
## 2025-05-24 - Missing Feishu Webhook Authentication
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` was entirely unauthenticated. It parsed the request body and directly relayed content to a Discord support channel, making the system vulnerable to spam, spoofing, and potential Server-Side Request Forgery (SSRF) if the payload structure was exploited.
**Learning:** Webhooks must always authenticate incoming requests. Feishu provides a verification token that is sent in the request body. Since Feishu sends different payload structures (v1 has token at root `body.token`, v2 has it nested at `body.header.token`), both locations must be checked. Also, if reading raw body text for signature or token validation in a Cloudflare Worker using `c.req.text()`, it is vital to wrap the subsequent `JSON.parse` in a `try...catch` block to handle malformed payloads without crashing the worker.
**Prevention:** Always require and validate verification tokens or signatures for webhook endpoints. Do not assume incoming JSON is well-formed or legitimate; use safe parsing blocks (`try...catch`) and validate tokens against securely injected environment secrets (e.g., using `sst.Secret`).

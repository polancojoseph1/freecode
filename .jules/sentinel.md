...
## 2026-05-06 - Command Injection Prevention in Electron IPC handlers
**Vulnerability:** Found a potential command injection and path traversal vulnerability in the `open-path` IPC handler in `packages/desktop-electron/src/main/ipc.ts` due to unsanitized application names being passed directly to `execFile` or `open -a`.
**Learning:** In Electron IPC handlers, user-controlled input like application names must be validated and sanitized. Simply passing them to execution functions can lead to execution of arbitrary binaries or path traversal. Always validate the input to reject path separators and verify the existence of the application using injected safe methods like `deps.checkAppExists` and `deps.resolveAppPath`. Note that automated code reviewers may incorrectly hallucinate the absence of methods on dynamically injected dependencies. Always trust local typechecking results (e.g., `bun turbo typecheck`) to verify property existence rather than relying solely on reviewer claims.
**Prevention:** Always validate application names passed from the renderer to prevent path separators. Use injected dependency methods to check app existence and resolve its safe path before executing.

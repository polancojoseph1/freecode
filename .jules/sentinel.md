...
## $(date +%Y-%m-%d) - Command Injection in Rust Command Exec Wrapper
**Vulnerability:** Command injection in `src-tauri/src/cli.rs` sidecar wrapper where args were interpolated directly into a shell string via `format!("\"{}\" {}", sidecar, args)` before being passed to `sh -l -c`.
**Learning:** Shell strings can trivially evaluate shell metacharacters in interpolated positional arguments. Using `shlex::split` to parse the `args` string into discrete tokens allows using `$0` `$@` forwarding pattern (`exec "$0" "$@"`) so that arguments are handed directly to the command as an array, completely bypassing shell interpretation.
**Prevention:** Never interpolate runtime arguments into a `sh -c` wrapper string. Parse string arguments into a vector (e.g. using `shlex`) and pass them using discrete arguments with `Command::new(shell).args(["-c", "exec \"$0\" \"$@\"", bin_name]).args(parsed_args)`.
## $(date +%Y-%m-%d) - Unit Tests Failing with Test Timeouts
**Learning:** For test files timing out unexpectedly during global test execution, look out for test suites increasing timeout lengths using explicit long running tests on fake or mocked properties (e.g. `vi.useFakeTimers()` breaking when transitioning to Bun test without using an actual fallback correctly). When migrating tests from Vitest to Bun, replacing `vi.useFakeTimers` with real timers (`Bun.sleep`) requires tests to be explicitly marked as `async`, and the assertions need to accurately account for the actual test execution and `sleep` delays.

...
## 2024-05-30 - Missing webhook signature validation
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` processed payloads without validating the signature. This could allow an attacker to spoof webhook payloads, leading to unauthorized message dissemination or spoofing. The signature check uses `x-lark-signature`, `x-lark-request-timestamp`, and `x-lark-request-nonce`.
**Learning:** Webhook endpoints must always validate the origin and integrity of incoming payloads, especially when they act as an intermediary for sensitive actions (like sending messages to internal Discord channels). The `Resource` definition for SST v3 does not auto-update, requiring manual additions across the monorepo to avoid compilation errors.
**Prevention:** Always implement signature verification on webhook endpoints before processing the payload. Explicitly verify the presence of required headers and use cryptographic timing-safe comparisons to avoid timing attacks.

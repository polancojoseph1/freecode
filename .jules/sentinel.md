...
## 2025-05-24 - Missing Webhook Signature Validation
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` did not validate incoming payloads against the `x-lark-signature` header, allowing unauthenticated attackers to forge webhook events.
**Learning:** Webhook endpoints must always cryptographically verify the origin of incoming requests. Wrapping signature verification in an if-block that allows requests without headers to fall through and process the payload creates a critical authentication bypass. Use `await c.req.text()` to extract the raw body string for precise cryptographic signature validation instead of `await c.req.json()`, and explicitly reject requests with missing headers.
**Prevention:** Implement strict signature validation by hashing the expected parameters (timestamp, nonce, secret, raw body) and securely comparing byte-lengths using `timingSafeEqual`. Ensure requests missing required signature headers are explicitly rejected.

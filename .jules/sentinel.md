...
## 2025-05-24 - Feishu Webhook Unauthenticated Vulnerability
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` processed requests without verifying the request's authenticity. This allowed an attacker to send forged payloads and impersonate Feishu, which led to arbitrary messages being relayed to the support channel.
**Learning:** Webhook endpoints that trigger actions on third-party integrations (like Discord) must authenticate incoming payloads to verify the sender's identity. Furthermore, Feishu's signature validation involves payload encryption if the encrypt key is enabled, which would break the integration if not implemented fully.
**Prevention:** Verify the plaintext `token` provided in the JSON body against a securely stored `FEISHU_VERIFICATION_TOKEN`. Always handle JSON parsing in a `try/catch` block to prevent unhandled exceptions on malformed payloads.

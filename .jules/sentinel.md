...
## $(date +%Y-%m-%d) - Timing Attacks in Secret String Comparisons
**Vulnerability:** The `assertSecret` and `share_delete_admin` endpoints in `packages/function/src/api.ts` used standard Javascript equality operators (`!==`) to compare user-provided secrets against stored secrets. This creates a timing attack vulnerability where an attacker can determine the correct secret character by character by measuring the response time.
**Learning:** In Javascript, `===` and `!==` operators fail fast when characters mismatch, leaking the index of the first incorrect character.
**Prevention:** Always use a constant-time comparison function, such as `crypto.timingSafeEqual` in Node.js (or Web Crypto API equivalents when applicable), and ensure that the lengths of the compared buffers are strictly validated before passing them to the equality check to avoid application crashes or DoS vectors.

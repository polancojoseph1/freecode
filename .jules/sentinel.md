...
## $(date +%Y-%m-%d) - Command Execution via Path Traversal in IPC Handlers
**Vulnerability:** The `open-path` IPC handler in Electron directly passed the `app` parameter from the renderer process into `execFile` without validation.
**Learning:** Even when `resolveAppPath` checks exist, pre-resolving the path in the renderer process and sending it back to the main process, or not verifying the application's actual existence and name format on the main process side before execution, can lead to arbitrary code execution or local privilege escalation via a compromised renderer.
**Prevention:** In Electron IPC handlers (`packages/desktop-electron`), never pass unsanitized executable names or paths from the renderer directly to `execFile` or `spawn`. Prevent path traversal and arbitrary execution by validating the application name (e.g., rejecting path separators via `/[/\\]/.test(app)` or explicitly allowing a strict regex pattern) before checking existence or resolving paths. Always use `await deps.checkAppExists` and `await deps.resolveAppPath` within the main process IPC handler.

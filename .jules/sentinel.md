...
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` processed external requests without verifying the request's authenticity or integrity, allowing an attacker to spoof Feishu bot events.
**Learning:** Webhook endpoints must always validate signatures to ensure they originate from the expected source. Relying merely on checking the existence of fields in the payload is insufficient for security. Hono's `c.req.text()` must be used to retrieve the exact, raw request body string for cryptographic hashing, as `c.req.json()` consumes the stream and doesn't provide the exact original bytes.
**Prevention:** Always implement the vendor's specified signature verification process (e.g., combining timestamp, nonce, secret, and raw body in a SHA-256 hash). Ensure early returns (e.g., 401 Unauthorized) if required signature headers are missing to prevent logic fallthrough. Use `timingSafeEqual` with strict buffer length checks to prevent timing attacks and Node.js exceptions during buffer comparison.

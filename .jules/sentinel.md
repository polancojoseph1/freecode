...
## 2025-02-20 - Missing Feishu Webhook Signature Validation
**Vulnerability:** The `/feishu` endpoint in `packages/function/src/api.ts` lacked signature validation, allowing unauthenticated attackers to spoof events.
**Learning:** Hono requires using `await c.req.text()` to extract the raw body string first for precise cryptographic signature validation (before parsing as JSON). Additionally, `timingSafeEqual` in Node.js requires both Buffers to be of the exact same length, otherwise it throws an exception. We must explicitly check `.length` equality before calling `timingSafeEqual`.
**Prevention:** Always enforce signature validation on webhook endpoints using an early return to reject missing headers, use raw text bodies for computing hashes, and validate Buffer lengths before securely comparing cryptographic signatures.

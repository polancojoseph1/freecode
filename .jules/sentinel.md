...
## 2025-09-03 - Feishu Webhook Signature Verification Implementation
**Vulnerability:** The Feishu webhook endpoint was unprotected, allowing anyone to post unauthorized messages by spoofing payloads to the `/feishu` endpoint.
**Learning:** Feishu's webhook verification uniquely requires generating a simple SHA-256 hash (`createHash("sha256")`, NOT HMAC) by concatenating `timestamp + nonce + secret + body` in that strict exact order. Furthermore, the final hash must be encoded as a hexadecimal string (`.digest('hex')`) instead of the base64 output commonly used by other platforms.
**Prevention:** When implementing webhook security for a third-party platform, always rigorously check their API documentation for their specific cryptographic primitives (like Hash vs HMAC) and parameter serialization order, as these details diverge significantly between vendors.

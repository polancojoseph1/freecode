...
## 2025-02-23 - Feishu Webhook Authentication Bypass
**Vulnerability:** The `/feishu` endpoint in `packages/function/src/api.ts` parsed the request payload and processed discord bot messages without verifying the `x-lark-signature` header from Feishu. This allowed arbitrary requests to trigger Discord notifications.
**Learning:** Cloudflare Workers/Hono webhooks need explicit signature verification early in the request handler. Using `await c.req.text()` to get the raw body string is crucial for proper hash computation since `c.req.json()` consumes the stream.
**Prevention:** Always implement signature verification (`x-lark-signature`) for external webhooks before parsing the body. When dealing with webhooks, compute standard hashes (e.g. `sha256` of `timestamp + nonce + secret + rawBody`) and use `timingSafeEqual` with explicitly matched buffer lengths to prevent timing attacks and Node buffer length errors.

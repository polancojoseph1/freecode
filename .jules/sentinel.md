...
## 2024-07-28 - Webhook Signature Validation Bypass
**Vulnerability:** The `/feishu` webhook endpoint lacked cryptographic signature validation, allowing anyone to send unauthenticated POST requests and trigger internal messages or spoof Discord support messages.
**Learning:** Even internal or third-party integration endpoints (like webhooks) must be treated as public, untrusted attack vectors. The lack of validation on `packages/function/src/api.ts` was a critical authentication bypass. Webhook validation logic should never allow requests to fall through if signature headers are missing.
**Prevention:** Always enforce webhook signature checks using `node:crypto`'s `createHash` (or `createHmac` depending on the provider) based on incoming headers (e.g., `x-lark-signature`, `x-lark-request-timestamp`, `x-lark-request-nonce`) and the raw request body (`await c.req.text()`). Explicitly reject requests (e.g., return 401) if required signature headers are missing.

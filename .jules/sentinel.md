...
## 2025-05-24 - Feishu Webhook Signature Validation Missing
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` processed incoming payloads without verifying the event request signature. This allowed unauthenticated attackers to spoof events and send arbitrary messages to the Discord support channel.
**Learning:** Webhooks receiving external events must explicitly validate the authenticity of the payload. Wrapping signature validation in an if-block or failing to early-return when required signature headers are missing allows authentication bypass.
**Prevention:** Always extract and validate the expected signature headers (e.g., `x-lark-signature`), using cryptographic comparison tools (like `timingSafeEqual`) against a computed hash of the raw body and secret. Explicitly reject (return 401) requests missing these required headers.

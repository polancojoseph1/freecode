...
## 2024-05-18 - Missing Webhook Authentication in Hono Endpoints
**Vulnerability:** The `/feishu` webhook endpoint lacked signature verification, allowing unauthenticated requests to be processed and forward arbitrary payloads to Discord or perform unauthorized actions.
**Learning:** Webhook endpoints must explicitly enforce signature validation and reject requests (e.g., with an early return 401) if the required signature headers are missing or invalid, using secure comparisons like `timingSafeEqual`.
**Prevention:** Ensure all webhook endpoints implement strict signature validation using the provided secret, reject requests missing headers early, use `timingSafeEqual` with byte-length checks, and never let unauthorized requests proceed to processing.

...
## $(date +%Y-%m-%d) - Missing Webhook Signature Validation
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` was processing incoming requests without verifying the `x-lark-signature`. This allowed potential attackers to forge webhook payloads and trigger unauthorized actions.
**Learning:** Webhook endpoints must explicitly validate the cryptographic signature provided by the external service before parsing or acting upon the payload. Wrapping verification in an if-block that allows unauthenticated requests to fall through creates an authentication bypass.
**Prevention:** Always extract the raw request body (e.g., `await c.req.text()`) to compute the hash exactly as the sender did, rather than using parsed JSON. Reject requests early (with HTTP 401) if required signature headers are missing, and use `timingSafeEqual` with byte-length checks to compare the expected and provided signatures to prevent timing attacks.

...
## 2025-05-24 - Feishu Webhook Security and Signature Validation Pattern
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` lacked signature validation, allowing anyone to send forged webhook payloads to the endpoint.
**Learning:** Feishu's webhook signature validation relies on computing a standard SHA-256 hash (`createHash("sha256")`), rather than an HMAC, using a concatenation of `x-lark-request-timestamp`, `x-lark-request-nonce`, the webhook secret, and the raw request body. The resulting digest must be hex-encoded and securely compared using `timingSafeEqual`.
**Prevention:** Always implement signature validation on public webhook endpoints. For Feishu specifically, remember that the signature scheme is a plain SHA-256 hash (not HMAC), and ensure constant-time comparison (e.g., `timingSafeEqual`) is used to verify the signature after explicitly verifying byte-lengths.

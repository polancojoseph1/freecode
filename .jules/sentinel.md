...
## 2024-05-18 - [CRITICAL] Missing Feishu webhook signature validation
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` lacked signature validation and early return mechanism, allowing arbitrary unauthenticated requests to be processed as valid webhooks.
**Learning:** External webhook endpoints require strict cryptographic signature validation using provided headers (e.g., `x-lark-signature`, `x-lark-request-timestamp`, `x-lark-request-nonce`) and the raw request body. The request should be rejected immediately (early return) if required headers are missing or if the computed signature does not match. Additionally, `await c.req.text()` must be used to get the raw body for accurate hashing before parsing as JSON. `timingSafeEqual` with matched buffer lengths should be used to prevent timing attacks.
**Prevention:** Implement signature verification with strict early returns (e.g., `if (!signature) return c.json(...)`) for all webhook endpoints. Use the Web Crypto API or Node's `crypto` module to securely compute hashes and compare them in constant time.

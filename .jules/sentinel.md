...
## 2025-05-24 - Missing Feishu Webhook Validation
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` lacked signature validation, allowing unauthorized external requests to forge webhook payloads and perform authenticated actions like sending messages to the internal Discord support channel.
**Learning:** Webhook payloads must always be authenticated using cryptographic signatures if provided by the vendor. When comparing signatures using `crypto.timingSafeEqual`, the byte lengths of both buffers must be explicitly compared before the function is called, otherwise Node.js throws an exception due to mismatched buffer lengths, leading to a denial-of-service.
**Prevention:** Always implement vendor-specific signature verification algorithms (like SHA-256 for Feishu). Perform manual buffer length equality checks (`buf1.length === buf2.length`) before calling `timingSafeEqual`.

...
## $(date +%Y-%m-%d) - Zombie Process / Timeout Leaks from Unbounded Sleeps in Async Tests
**Vulnerability:** In `packages/opencode/test/session/retry.test.ts`, the `Bun.serve` mock server used an explicit `await sleep(10000)` combined with `test(..., 15_000)` to simulate long-running network operations for `ECONNRESET` handling. However, hanging open sockets artificially inflated test execution time, occasionally pushing overall CI job limits (which can kill other tasks unceremoniously, manifesting as a `143` exit code) and causing resource leaks.
**Learning:** Hard-coded `sleep()` commands in mock servers to simulate disconnections are risky because they extend total test duration unnecessarily, especially if teardown doesn't sever the connections immediately.
**Prevention:** Instead of using long sleep durations, explicitly stop the server mid-stream using `server.stop(true)` triggered via `setTimeout` in the `fetch` handler. This provides deterministically fast test execution (simulating connection drops natively) and avoids artificially long network waits.

...
## 2025-05-24 - Feishu Webhook Signature Validation Bypass
**Vulnerability:** The Feishu webhook endpoint at `/feishu` in `packages/function/src/api.ts` was not validating incoming webhook request signatures (`x-lark-signature`), which could allow an attacker to spoof incoming messages or actions without authorization.
**Learning:** Forgetting to implement signature verification in webhook handlers is a common security omission that leaves the integration endpoints wide open.
**Prevention:** Implement signature validation by securely comparing the expected hash of the timestamp, nonce, secret, and raw body with the provided `x-lark-signature` header using `timingSafeEqual` and explicit byte-length checks to prevent bypasses via timing attacks or multi-byte string mismatches. Ensure to always use `c.req.text()` to grab the raw unparsed body for hashing.

...
## 2025-06-26 - Feishu Webhook Signature Validation Bypass
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` processed incoming payloads without verifying the webhook signature (`x-lark-signature`). This allowed unauthorized parties to forge payloads and potentially trigger discord support channel messages or bypass authentication.
**Learning:** Webhook endpoints must always validate the signature of incoming requests to ensure they originate from the expected service and haven't been tampered with. If the headers are missing, the request should be immediately rejected with an early return before consuming the payload. Also, always ensure the payload is read with `await c.req.text()` for precise cryptographic signature validation instead of `await c.req.json()` which parses the request and modifies the raw payload.
**Prevention:** Implement webhook signature validation early in the handler using cryptographic hashing (e.g. `crypto.createHash("sha256")`). Use `timingSafeEqual` and explicitly compare the byte-lengths of the generated Buffers to prevent bypasses and Node.js exceptions. Ensure to explicitly return a 401 error if signature headers are missing or invalid, avoiding fallthrough logic.

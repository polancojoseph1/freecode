...
## 2025-07-31 - Missing Authentication on Feishu Webhook
**Vulnerability:** The Feishu webhook endpoint (\`/feishu\`) in \`packages/function/src/api.ts\` was processing payloads and sending messages to a Discord support channel without any authentication or signature validation. This could allow an attacker to spoof Feishu events and send arbitrary messages to the Discord support channel by sending unauthenticated POST requests to the endpoint.
**Learning:** Webhook endpoints exposed to the internet must validate signatures using a secret to ensure requests originated from the trusted service (e.g., Feishu). For Feishu, the signature is a SHA-256 hash of the timestamp, nonce, secret, and raw request body. Also, it's essential to use a constant-time comparison function (e.g., \`timingSafeEqual\`) to avoid timing attacks when comparing signatures.
**Prevention:** Always implement signature validation for webhooks. Read the raw request body as text, parse the required signature headers, compute the expected hash using the configured webhook secret, and compare the hashes securely using \`crypto.timingSafeEqual\` before parsing and processing the JSON payload. Ensure requests are rejected with a 401 status if headers are missing or signatures do not match.

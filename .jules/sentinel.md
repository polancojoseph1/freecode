...
## 2025-06-25 - Unauthenticated Feishu Webhook
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` did not validate incoming requests. This allowed any unauthenticated attacker to impersonate Feishu, trigger the webhook, and send arbitrary messages to the Discord support channel on behalf of the bot, creating a spoofing and potentially abusive capability.
**Learning:** Feishu webhooks use a custom signature mechanism via the `x-lark-signature`, `x-lark-request-timestamp`, and `x-lark-request-nonce` headers. The signature is computed as a raw SHA-256 hash (NOT HMAC) of the string `timestamp + nonce + secret + rawBody`. Additionally, when validating signatures in Hono, extracting the raw body stream requires `await c.req.text()`. Doing this after or before `await c.req.json()` fails because the stream can only be read once.
**Prevention:** Always implement signature validation for incoming webhooks. Specifically for Hono routes requiring body signature validation, extract the body using `await c.req.text()` to accurately verify the hash before manually parsing it with `JSON.parse()`. Use `timingSafeEqual` with strict length checks to prevent timing attacks when comparing hashes.

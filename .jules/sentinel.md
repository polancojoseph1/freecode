...
## 2025-02-27 - [Security Enhancement] Authenticate Feishu Webhook with SHA-256 Signatures
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` accepted unauthenticated requests from anyone, risking arbitrary data injection or spoofing of discord support messages.
**Learning:** Feishu webhooks authenticate payloads via an `x-lark-signature` header, generated by a SHA-256 hash of `timestamp + nonce + WEBHOOK_SECRET + rawBody`. Because the body must exactly match the incoming bytes, you must parse the raw text before parsing the JSON for calculating the hash. `node:crypto`'s `timingSafeEqual` should be used with strict byte-length comparisons of the Buffers to prevent timing attacks.
**Prevention:** Always implement signature verification on incoming webhooks with secret keys. Never rely solely on obscurity (a hard-to-guess URL or missing documentation). When comparing signatures or hashes, strictly use cryptographic timing-safe comparisons and raw bodies.

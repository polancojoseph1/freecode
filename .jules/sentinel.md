...
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` lacked signature validation and processed incoming requests blindly.
**Learning:** This could allow unauthorized requests, bypassing authentication and potentially sending spoofed messages on behalf of the application, such as via the Discord support bot integration. The fix involved extracting the `x-lark-signature`, `x-lark-request-timestamp`, and `x-lark-request-nonce` headers, calculating the expected signature using SHA-256 HMAC (with the payload and webhook secret), and securely comparing them with `timingSafeEqual`.
**Prevention:** Always validate webhook signatures using `timingSafeEqual` and explicitly return an early error (e.g. 401) if required signature headers are missing or if the signatures do not match.

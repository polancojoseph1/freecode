...
## 2025-05-25 - Webhook Authentication Bypass Prevention
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` completely lacked signature verification, allowing unauthenticated attackers to forge webhook payloads and spoof Feishu messages into the Discord support channel.
**Learning:** Webhook endpoints must explicitly enforce authentication using signature headers before processing the request body. Neglecting this opens the door to arbitrary event spoofing and potential SSFR/RCE depending on downstream handling. Additionally, computing signatures with standard hashes and comparing them using non-constant time checks can be vulnerable to timing attacks.
**Prevention:** Always validate webhook signatures. Ensure that requests without required headers are explicitly rejected (early return 401). When verifying cryptographic hashes, use `timingSafeEqual` to compare signatures and explicitly verify buffer byte-lengths to prevent Node.js mismatch exceptions.

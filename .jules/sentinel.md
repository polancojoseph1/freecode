...
## $(date +%Y-%m-%d) - Feishu Webhook Signature Validation Pattern
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` lacked signature validation, allowing arbitrary attackers to send forged webhook payloads to the application.
**Learning:** Feishu's webhook signature validation relies on a unique pattern. Instead of a typical HMAC, it computes a plain SHA-256 hash (`createHash("sha256")`) using the exact order: `timestamp + nonce + Webhook Secret + rawBody`. The resulting hash must be encoded as a hexadecimal string (`.digest("hex")`) and safely compared against the `x-lark-signature` header using `timingSafeEqual`.
**Prevention:** Always secure public webhook endpoints. When implementing Feishu webhooks, verify the signature exactly as prescribed by their documentation, using a dedicated webhook secret distinct from the App Secret.

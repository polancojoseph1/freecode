...
## $(date +%Y-%m-%d) - Webhook Endpoint Authentication
**Vulnerability:** The `/feishu` webhook endpoint lacked any form of token authentication or request verification, leaving the endpoint susceptible to accepting unauthorized payloads from arbitrary sources.
**Learning:** For endpoints acting as webhooks (such as the Feishu endpoint receiving internal event data), relying purely on the difficulty of finding the endpoint URL (security by obscurity) is insufficient. External payloads can be trivially spoofed if not cryptographically validated or authenticated using pre-shared secrets.
**Prevention:** Always authenticate webhook calls using pre-shared verification tokens (and ensure these tokens are securely compared using `crypto.timingSafeEqual` in Node/Bun to prevent timing attacks) or via HMAC-based request signature validation where provided by the platform.

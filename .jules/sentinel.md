...
## 2024-05-14 - Missing Webhook Signature Validation on `/feishu` Endpoint
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` was missing signature verification. Anyone could send a POST request with a forged payload, and it would be accepted and processed.
**Learning:** External webhook endpoints require cryptographic verification to ensure the request actually originated from the expected service (in this case, Feishu/Lark) and hasn't been tampered with. Without it, there is a severe risk of spoofing and unauthorized actions.
**Prevention:** Always implement signature validation for webhooks. For Feishu, verify the `x-lark-signature` header using `createHash("sha256")` on `timestamp + nonce + secret + rawBody`, and perform a constant-time comparison (e.g. `timingSafeEqual`) on the resulting buffers after asserting matching lengths. Ensure to reject requests early if the required headers are missing.

...
## 2024-06-28 - [CRITICAL] Missing webhook authentication
**Vulnerability:** The `/feishu` webhook endpoint lacked cryptographic signature validation. It parsed the request body directly without verifying the `x-lark-signature`, `x-lark-request-timestamp`, and `x-lark-request-nonce` headers. This allowed unauthenticated attackers to forge arbitrary webhook events by sending POST requests to the endpoint, bypassing authentication entirely.
**Learning:** In Hono, Express, and similar web frameworks, if signature verification logic is missing or improperly implemented (e.g., wrapped in an if-block allowing fall-through), anyone can invoke the webhook handler. Furthermore, to compute the signature securely, the exact raw body string must be used (`await c.req.text()`) rather than the parsed JSON object to avoid serialization differences.
**Prevention:** Always implement signature validation for external webhooks. Ensure the validation explicitly rejects requests with missing headers (early return 401). Use the exact raw body for hashing, decode the expected signature matching the protocol (e.g. hex), and perform a constant-time comparison (`timingSafeEqual`) after explicitly checking buffer lengths.

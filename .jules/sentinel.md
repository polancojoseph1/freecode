...
## 2024-03-24 - Prevent Arbitrary Command Execution in IPC openPath
**Vulnerability:** The Electron `ipcMain` handler for `open-path` executed unsanitized, renderer-provided application paths directly via `execFile`, leading to potential arbitrary command execution. Additionally, the renderer attempted to pre-resolve application paths before sending them to the main process, bypassing secure validation.
**Learning:** In Electron, never pass unsanitized executable names or paths from the renderer directly to `execFile` or `spawn`. The main process must fully control the resolution and validation of application paths.
**Prevention:** Validate application names using a strict allowlist or strict regular expression (e.g., `/^[a-zA-Z0-9\-\_\. ]+$/`) in the main process to prevent shell metacharacter injection. Send only the raw application name string from the renderer, and use the main process's dependency injection to securely verify and resolve the executable path before execution.

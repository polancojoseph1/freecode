...
## 2024-05-18 - Missing Webhook Signature Validation on Feishu Endpoint
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` processed incoming payloads without verifying the webhook signature (`x-lark-signature`), allowing potential spoofing of webhook requests and bypassing authentication.
**Learning:** The signature validation requires generating a SHA-256 hash using `x-lark-request-timestamp`, `x-lark-request-nonce`, the secret, and the raw request body. Secure comparison of the hashes must use `crypto.timingSafeEqual` and byte-length checks to prevent timing attacks and Node.js buffer length exceptions. Also, retrieving the raw body correctly in Hono is necessary for accurate hash computation instead of using the parsed JSON.
**Prevention:** Always enforce signature validation on webhook endpoints before parsing and processing the body payload. Ensure the comparison utilizes `timingSafeEqual` with matching buffer lengths.

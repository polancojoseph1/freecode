...
## 2024-05-18 - Webhook Signature Validation Bypass via Missing Headers
**Vulnerability:** The `/feishu` webhook endpoint lacked cryptographic signature validation entirely, allowing forged webhooks. When adding signature validation, if the logic does not explicitly check for the presence of required headers (e.g., `x-lark-signature`) and reject requests without them before body parsing, an authentication bypass can occur.
**Learning:** Webhook validation using `timingSafeEqual` or hash comparisons must guarantee that expected headers are actually provided. Allowing execution to fall through if headers are undefined causes validation bypass. Also, `await c.req.text()` is required in Hono to extract the raw body strictly needed for signature checking without consuming the stream twice.
**Prevention:** Always use early returns (e.g. 401 Unauthorized) when webhook signature headers are missing, verify buffer lengths before `timingSafeEqual`, and manually parse `JSON.parse(await c.req.text())` when signature hashing is needed.

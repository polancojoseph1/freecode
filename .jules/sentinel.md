...
## 2024-06-24 - [CRITICAL] Webhook Signature Bypass Prevented

**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` lacked signature verification, allowing anyone to forge requests to the endpoint and spam the Discord support channel. Additionally, it read the body using `await c.req.json()`, which consumes the stream, making the raw body unavailable for future validation.
**Learning:** Webhook endpoints must always validate signatures, explicitly rejecting missing headers immediately rather than allowing them to fall through. For accurate hash generation, you must extract the raw body exactly as received (using `await c.req.text()`) before performing any JSON parsing. Furthermore, when generating signatures in Cloudflare Workers, `Buffer` must be explicitly imported (`import { Buffer } from "node:buffer"`).
**Prevention:** Always mandate header validation (`timestamp`, `nonce`, `signature`) and explicitly read raw streams first (`await c.req.text()`) to verify payloads against shared secrets, before parsing as JSON. Compare signature hashes using `timingSafeEqual`, ensuring matching lengths before comparison.

...
## 2026-09-23 - DOMPurify usage for XSS Prevention\n**Vulnerability:** XSS vulnerability in web components due to unsanitized HTML being passed to `innerHTML`.\n**Learning:** When using solid-js `innerHTML` or React's `dangerouslySetInnerHTML`, the input string must be sanitized to prevent malicious scripts from executing. In a web environment, `dompurify` is an effective tool for this.\n**Prevention:** Always sanitize dynamically generated HTML (like markdown or syntax-highlighted code output) using `DOMPurify.sanitize` before rendering it in the DOM.

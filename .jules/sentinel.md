...
## 2025-05-24 - Feishu Webhook Signature Validation Failure-Open
**Vulnerability:** Feishu webhook signature validation missing implementation.
**Learning:** When adding third-party webhook integrations, standard signature validation headers MUST be checked. Leaving endpoints open or missing validation logic allows attackers to spoof requests.
**Prevention:** Implement HMAC or appropriate signature checks (like `createHash('sha256')` with `timingSafeEqual`) and fail-closed by rejecting requests missing required validation headers.

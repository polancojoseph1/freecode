...
## $(date +%Y-%m-%d) - Webhook Signature Bypass via String Equality
**Vulnerability:** The `/feishu` webhook endpoint lacked cryptographic signature verification entirely, and typical implementations often use standard string equality (`===`) when comparing incoming signatures with expected hashes.
**Learning:** Comparing cryptographic hashes as strings with `===` is vulnerable to timing attacks, as the comparison returns early upon finding the first mismatched character. Furthermore, using `timingSafeEqual` directly on strings or unequally-sized Buffers can throw an exception or be bypassed if multi-byte characters alter the perceived length vs byte size.
**Prevention:** Always compare webhook signatures using `timingSafeEqual` on `Buffer` objects, and explicitly verify that `Buffer.from(signature).length === Buffer.from(expectedSignature).length` before comparison to ensure uniform execution time and prevent bypasses. Ensure you use `c.req.text()` instead of `c.req.json()` to capture the exact raw body for signature generation.

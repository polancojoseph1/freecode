...
## 2024-06-29 - [CRITICAL] Missing Webhook Signature Validation
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` was missing cryptographic signature validation for incoming payloads, allowing any request to trigger the discord support bot impersonation.
**Learning:** The validation logic must explicitly compute a SHA-256 hash using `x-lark-request-timestamp`, `x-lark-request-nonce`, `FEISHU_WEBHOOK_SECRET`, and `rawBody`. We must use `await c.req.text()` to extract the raw body unaltered instead of relying on `await c.req.json()` which parses and serializes it, breaking the hash match.
**Prevention:** Always implement webhook signature validation by strictly verifying the required header elements using raw payload values and use `timingSafeEqual` after comparing the length of the buffers. Ensure new secrets like `FEISHU_WEBHOOK_SECRET` are added securely via SST infrastructure (`infra/app.ts`) and appropriately typings (`sst-env.d.ts`).

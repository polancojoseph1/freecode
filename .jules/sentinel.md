...
## 2024-05-24 - [Fix missing authentication on Feishu webhook endpoint]
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` lacked signature validation and processed all incoming payloads unconditionally without verifying `x-lark-signature`, leading to a critical authentication bypass.
**Learning:** In Hono, extracting the request body using `await c.req.json()` consumes the stream. Validating signatures often requires the exact raw payload. Failing to check required headers early can leave the endpoint exposed.
**Prevention:** Always extract `await c.req.text()` first for signature validation, then manually `JSON.parse()`. Implement an early return (`401 Unauthorized`) if required signature headers (`x-lark-request-timestamp`, `x-lark-request-nonce`, `x-lark-signature`) are missing. Use `timingSafeEqual` with byte-length validation instead of naive string matching to prevent timing attacks.

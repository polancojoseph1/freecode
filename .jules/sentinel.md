...
## 2024-05-18 - Missing Webhook Signature Validation on `/feishu`
**Vulnerability:** The `/feishu` webhook endpoint lacked cryptographic signature validation. It directly parsed the incoming JSON body (`await c.req.json()`) without verifying the `x-lark-signature`, `x-lark-request-timestamp`, or `x-lark-request-nonce` headers. This allowed an unauthenticated attacker to spoof Feishu event payloads by sending crafted POST requests directly to the endpoint.
**Learning:** In Hono, consuming the request body with `c.req.json()` prevents subsequent raw access. To perform HMAC or hash-based signature validation, the raw string body must be captured first using `await c.req.text()`, then manually parsed with `JSON.parse()`. Furthermore, when using `timingSafeEqual`, the byte-lengths of the two generated Buffers must be explicitly compared before execution; otherwise, multi-byte characters or mismatched lengths will bypass the check or cause Node.js to throw a `TypeError`, leading to denial of service.
**Prevention:** Always implement signature verification on webhook endpoints before processing the payload. Use `await c.req.text()` to extract the exact byte-for-byte representation of the request body for hashing, ensure the required headers are present, explicitly compare buffer lengths before using `timingSafeEqual`, and configure external secrets (like `FEISHU_WEBHOOK_SECRET`) in the infrastructure layer (`infra/app.ts`) to be injected into the worker runtime.

...
## 2025-02-27 - Unauthenticated Feishu Webhook Endpoint
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` lacked signature validation, allowing anyone to send unauthenticated requests and trigger actions (like sending messages to Discord) by simply mimicking the payload structure.
**Learning:** The endpoint processed the JSON body directly without verifying the origin of the request.
**Prevention:** Always implement signature validation for public webhook endpoints. For Feishu webhooks, compute the SHA-256 hash of `timestamp + nonce + secret + body` and use a constant-time comparison against the `x-lark-signature` header before parsing the payload.

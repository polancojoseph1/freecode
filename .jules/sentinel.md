...
## 2024-03-24 - Feishu Webhook Missing Signature Validation
**Vulnerability:** The `/feishu` webhook endpoint in `packages/function/src/api.ts` was blindly trusting incoming webhook payloads without validating the `x-lark-signature` header, allowing attackers to spoof events and send arbitrary messages to the Discord support channel.
**Learning:** External webhooks often include cryptographic signatures to prove authenticity, but developers sometimes omit validation, especially during initial integration. For Feishu webhooks, the signature is a SHA-256 hash (not HMAC) of the timestamp, nonce, webhook secret, and raw body (in that order), base64 encoded.
**Prevention:** Always validate webhook signatures. Ensure that requests lacking required signature headers are rejected immediately with a 401 response (early return) rather than allowing them to fall through. Use `timingSafeEqual` with explicitly matched buffer lengths to prevent timing attacks when comparing signatures. In Hono, use `c.req.text()` to get the raw body string for hashing before parsing it manually with `JSON.parse()`.

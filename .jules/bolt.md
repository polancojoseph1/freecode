...
## 2025-06-03 - [Optimizing ID Timestamp Extraction]
**Learning:** Native `parseInt(hex, 16)` is significantly faster than parsing and dividing `BigInt` values in V8/Bun environments. If a hex string safely fits within `Number.MAX_SAFE_INTEGER` (like a 12-character hex), `parseInt` should be preferred. Additionally, avoiding `split("_")` and using `indexOf("_")` for extracting parts of strings prevents unnecessary array allocations in hot paths.
**Action:** When working with hex string decoding where the value fits within 53 bits (`Number.MAX_SAFE_INTEGER`), avoid `BigInt`. For simple string segmenting on a delimiter, use `indexOf` or native string traversal methods instead of `split` to minimize allocations, particularly in frequently called utilities like ID parsing.

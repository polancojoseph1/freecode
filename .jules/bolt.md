...
## 2025-07-09 - [Performance] Native Math vs BigInt for timestamp extraction
**Learning:** For timestamp extraction from hex strings in high-frequency functions, avoiding `.split()` to prevent intermediary array allocations and replacing `BigInt` parsing and division with native `parseInt` and `Math.floor` division provides a ~10-15x performance increase. This is safe because a 12-character hex string fits safely within `Number.MAX_SAFE_INTEGER`.
**Action:** When extracting data from strings or performing calculations that fit within `Number.MAX_SAFE_INTEGER`, avoid heavy higher-order functions and BigInt if native Math and primitive operations can achieve the same result.

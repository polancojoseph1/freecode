...
## 2024-07-17 - Optimize ID generation and extraction
**Learning:** The ID generation functions were allocating BigInts unnecessarily. The max generated value `Date.now() * 0x1000 + counter` is safely below `Number.MAX_SAFE_INTEGER`. In V8/Bun, using `parseInt(hex, 16)` and native `Number` math with `Math.floor` division drops execution time significantly (e.g. from 1600ms -> 600ms for 1M iterations, and extracting timestamps from 440ms -> 45ms).
**Action:** When a hex string length guarantees parsing into a safe int (<= 12 chars usually), replace BigInt parsing with `parseInt(hex, 16)`. Use standard JavaScript division wrapped in `Math.floor()` to simulate BigInt truncating division, and replace BigInt bitwise shifts with exponent division to avoid the overhead of heavy big int allocation.

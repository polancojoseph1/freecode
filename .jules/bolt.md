...
## 2025-03-02 - Optimize string/BigInt conversion for ID timestamps
**Learning:** Native `parseInt(hex, 16)` with Numbers is significantly faster than using `BigInt` for parsing and dividing hex strings in the V8/Bun engine. If the hex string is known to represent a value that fits within `Number.MAX_SAFE_INTEGER` (e.g., up to 13 hex characters, 52 bits), you should prefer standard JS Numbers over `BigInt` to maximize performance. Furthermore, using `String.prototype.indexOf()` and `.slice()` instead of `.split()` prevents unnecessary intermediary array allocations.
**Action:** Always check the maximum possible value when dealing with `BigInt`. If it strictly fits in 53 bits (e.g., extracting 48-bit timestamps), refactor to `Number` and `parseInt` for hot-path ID processing. Avoid `.split()` when only the first delimiter index is needed.

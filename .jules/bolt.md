...
## 2024-05-23 - Base64 encoding/decoding performance
**Learning:** In Node/Bun environments, native `Buffer.from(value).toString("base64")` and `Buffer.from(value, "base64").toString("utf-8")` are astronomically faster (~20-25x) than manual byte conversion (using `TextEncoder`/`TextDecoder` alongside string array mapping). For browser fallbacks, manual iteration with `Array.from()` maps across potentially thousands of items which severely degrades performance; chunked `String.fromCharCode.apply` with standard loops performs significantly better without blocking the main thread.
**Action:** When implementing any base64 encoding/decoding string transformations in utilities shared between backend/frontend, always branch with `typeof Buffer !== "undefined"` to leverage native optimization, and avoid `Array.from` when iterating bytes for the fallback.

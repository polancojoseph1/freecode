...
## 2024-05-20 - BigInt vs parseInt Performance for ID Decoding
**Learning:** In V8/Bun environments, native `parseInt(hex, 16)` is significantly faster than parsing and dividing `BigInt` values. For ID generation/decoding systems using 12-character hex timestamps, the max value (`0xFFFFFFFFFFFF`) fits safely within `Number.MAX_SAFE_INTEGER`. However, when migrating from `BigInt` division to `Number` division, you must explicitly use `Math.floor()` to replicate `BigInt`'s integer truncation, as JavaScript's standard division yields floating-point results.
**Action:** When optimizing decoding algorithms involving moderately sized hex strings, prefer `parseInt` and `Math.floor` over `BigInt` allocations for performance, provided the values remain strictly within safe integer limits.

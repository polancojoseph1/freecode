...
## 2026-04-17 - [Database Side-Effects Optimization]
**Learning:** In `packages/opencode`, performing asynchronous side-effects like `Bus.publish` sequentially or by directly `await`ing `Promise.all` immediately after database operations blocks the main execution flow unnecessarily.
**Action:** Always wrap asynchronous event notifications (`Promise.all(Bus.publish(...))`) inside `Database.effect(async () => { ... })` when running within a `Database.use(...)` transaction block to defer side-effects until after the transaction succeeds and unblock the current thread.

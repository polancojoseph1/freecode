...
\n## 2025-05-24 - [Avoid Sequential Config Array Mapping]\n**Learning:** When generating multiple configuration arrays that require strict deep-merge sequence, do not evaluate each configuration layer via a loop. The mapping function allows you to execute these loops concurrently in a single root Promise.all array.\n**Action:** Assign loop results to distinct promise arrays and bundle them into an overarching Promise.all(...) before iterating sequentially to enforce precedence order.

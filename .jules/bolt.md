...
## 2025-03-24 - [Ripgrep generator child process kill]
**Learning:** In async generators yielding from `proc.stdout`, breaking from the generator does not close the stream and leaves the `proc` hanging and tests timing out.
**Action:** When creating async generators from child process stdout, always wrap the loop and `exited` wait in a `try...finally` block that explicitly calls `proc.kill()` so early breaks terminate the process.

...
## 2025-03-08 - String processing without Array allocation
**Learning:** For small path manipulation strings, using `.replace` combined with `.split()` generates multiple array objects and strings, leading to excessive GC and allocations. Standard iteration through `charCodeAt` performs significantly faster without risking string copy or array initializations.
**Action:** When implementing small string extraction utility functions, prefer backward or forward iteration using `charCodeAt` to avoid multi-array intermediary allocations when splitting isn't absolutely required.

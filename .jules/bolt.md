...
## 2025-03-24 - [Avoid Micro-optimizations on Static Small Arrays]
**Learning:** Refactoring array manipulations (`.filter().map()`) to loops is only beneficial for dynamic or large arrays. Applying this to statically sized micro-arrays (e.g., 5 elements) provides no measurable impact and violates the rule against meaningless micro-optimizations, while also risking rejection during code review.
**Action:** When replacing array higher-order functions with loops to reduce object and closure allocations, ensure the target array is sufficiently large or part of a proven hot-path. Do not optimize small, fixed-size arrays.

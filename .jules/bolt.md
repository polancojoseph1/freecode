...
## 2025-05-24 - Zero-Allocation Path Manipulation in Bun/Node
**Learning:** Native V8 string methods like `split()` allocate intermediate arrays, which become a critical bottleneck for frequently called path utility functions. Using manual string traversal with `charCodeAt()` avoids these allocations completely. For operations where backwards lookup is sufficient (like finding a filename or directory from a path), traversing backwards and mapping indices for a single `.slice()` call is dramatically faster than creating multiple temporary arrays and joining them.
**Action:** For hot-path string utilities, prefer native methods like `lastIndexOf()` or manual index tracking via `charCodeAt()` over `.split()`, `.replace()` with Regex, or `.slice()`/`.join()` combinations to minimize garbage collection overhead.

...
## 2025-03-24 - [Path utility optimization with manual backward iteration]
**Learning:** Using regex like `.replace(/[\/\\]+$/, "")` followed by `.split(/[\/\\]/)` to manipulate paths heavily allocates intermediate arrays and strings which causes performance bottlenecks on hot paths handling large numbers of files.
**Action:** When creating high performance path manipulation functions (like `getDirectory` or `getFilename`), prefer backward manual iteration using raw string indexing (e.g. `path[end] === '/'`) over native `.split()` or regex parsing to balance peak performance with readability. Ensure you verify edge cases like root paths (`/`), empty paths, and multiple trailing slashes using the existing test suite.

...
## 2024-05-17 - Optimize base64Encode and base64Decode string conversion
**Learning:** In string processing (like base64 encoding), using `Array.from()` to map `Uint8Array` to an array of characters and then joining them is extremely slow in v8/Bun due to intermediate array allocation and repeated function calls. `String.fromCharCode.apply` with chunking, or even better, native `Buffer.from()` when available in Node/Bun environments, provides massive speedups (e.g., ~40x faster for `base64Encode`).
**Action:** When working with base64 conversions or string/byte conversions, prioritize native `Buffer` APIs if available. If falling back to the browser, avoid `Array.from()` for byte transformations and use chunked `String.fromCharCode.apply` or direct tight loops instead.

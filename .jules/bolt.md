...
## 2025-03-24 - [Path utility optimization with string methods]
**Learning:** Using regex and intermediate array transformations (like `.split()` and `.join()`) for high-frequency path manipulations adds significant allocation and processing overhead. While manual `charCodeAt` iteration for every character is too complex and reduces readability, using native string methods like `.replace()`, `.lastIndexOf()`, and `.slice()` provides an excellent balance of performance (2x-3x speedup) and readability.
**Action:** When writing string or path manipulation utilities, prefer using native string methods (e.g., `.lastIndexOf()`) over regex/array conversions or full manual C-style parsing.

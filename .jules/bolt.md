...
## 2025-03-24 - [Array.filter inside loops]
**Learning:** Using `Array.prototype.filter` inside of hot-path loops (like iterating through patch hunks to count additions and deletions) forces the allocation of intermediate arrays and closure functions on every iteration, causing significant garbage collection overhead.
**Action:** When counting or aggregating values in hot loops, replace `reduce` and `filter` chains with standard zero-allocation `for` loops and localized counter variables. Use `.charCodeAt(0)` instead of `.startsWith` for character checks to further avoid temporary string allocations.

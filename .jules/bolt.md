...
## 2025-01-20 - Optimize git diff patch parsing via zero-allocation iteration
**Learning:** Chaining array methods like `.reduce().filter().startsWith()` on potentially large and frequently accessed arrays (like lines within git diff hunks) causes unnecessary performance overhead due to the allocation of intermediate arrays for `.filter()` and new function closures.
**Action:** When performing simple aggregate counts across string arrays on hot paths or large datasets, replace chained methods with standard nested `for` loops. Check string prefixes natively with `.charCodeAt(0)` (e.g., `43` for `+`, `45` for `-`) instead of `.startsWith()` to eliminate allocations and dramatically reduce garbage collection pressure.

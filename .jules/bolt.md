...
## 2025-03-08 - Parallelizing configuration file loading
**Learning:** In the codebase's startup sequence (e.g., `packages/opencode/src/config/tui.ts`), configurations from various sources (global, custom, project, directory, managed) are layered sequentially to enforce priority rules. Previously, these were awaited sequentially using separate `Promise.all` blocks. When parallelizing multiple independent async I/O operations that must be merged in a specific order, it is optimal to hoist the instantiation of all promise structures and await them concurrently using an outer `await Promise.all([Promise.all(group1), group2, ...])` before sequentially iterating through the results to merge them.
**Action:** When optimizing multi-layered file loading or configuration resolution, group promises by priority level and execute them all concurrently via a single outer `Promise.all` to avoid N+1 blocking I/O, then preserve logical priority by merging the results sequentially.

...
## 2025-03-08 - Fast 53-bit Integer Extraction in V8/Bun
**Learning:** BigInt allocation is extremely slow in V8/Bun compared to standard 64-bit floating-point Numbers. When dealing with numbers generated via `timestamp * 0x1000 + counter` (which maximally hit ~53 bits), they fit safely inside `Number.MAX_SAFE_INTEGER`. Extracting bytes using floating point division and bitwise shifts (`Math.floor(num / Math.pow(2, shift)) & 0xff`) is significantly faster than using `BigInt` operations and allocation. Furthermore, `parseInt(hex, 16)` is substantially faster than `BigInt(hex)` parsing.
**Action:** When extracting bytes or parsing integers from hex that are known to fit within 53 bits (e.g. custom Snowflake-like IDs), use `Number`, `Math.floor`, and `parseInt` instead of `BigInt` to maximize performance and avoid unnecessary heap allocations.

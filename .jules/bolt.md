...
## 2026-09-22 - Reverted bun.lock modifications during tool testing
**Learning:** `bun install` or similar actions can unintentionally modify `bun.lock` and generated SDK files, which introduces severe, out-of-scope breaking regressions. Specifically, SDK client files in `packages/sdk/js/src/.../gen/...` can have logic fundamentally altered, leading to silent error swallowing, which will fail code review.
**Action:** Always verify changes meticulously using `git status` or `git diff --cached` before considering a task complete. Explicitly revert unintended staging of file modifications (e.g., using `git restore --staged <file>` and `git checkout -- <file>`) to prevent unapproved logic shifts outside of the targeted module.
## 2026-09-22 - Flaky async tests hanging in GitHub Actions
**Learning:** If specific asynchronous tests fail intermittently via timeout on GitHub actions (e.g., `tool.registry > loads tools...` and `tool.edit > editing existing files...`), these issues often manifest due to heavy concurrency or slow infrastructure operations causing them to cross the default 30,000ms threshold, especially when `npx turbo run test` runs full suites.
**Action:** Since these tests pass manually and quickly in isolated, less-contended local runs, and the CI log explicitly reports timeout (`this test timed out after 30000ms.`), they are identified as flaky testing infrastructure issues and unrelated to the targeted performance changes. Do not modify test execution limits or logic blindly, but monitor for systematic patterns.
## 2026-09-22 - GitHub Actions timeouts due to heavy test loads
**Learning:** Certain async tests, particularly those involving plugin installations (like `auth-override.test.ts` or `registry.test.ts`), are subject to heavy infrastructure contention on GitHub CI servers. They frequently cross the default test timeouts, triggering runner shutdown signals (exit code 143/cancellation). This causes flaky failures that are incorrectly flagged as regressions.
**Action:** Do not attempt to debug or modify application logic to "fix" timeouts that only occur in the CI environment for previously established test suites. In extreme timeout cases isolated to single tests, it is permissible to raise the test-specific timeout limit (`test("...", async () => {...}, 90000)`), but as the Bolt persona resolving a pure performance regression, strictly avoid making arbitrary test modifications outside the scope of the chosen optimization. Let CI retry logic handle it.

...
## 2025-03-08 - String Operation Performance in Utility Functions
**Learning:** In highly trafficked utility functions like path parsing (`getFilename`, `getDirectory`), utilizing Regex replacements and array `split`/`join` operations results in high memory allocation and CPU overhead in V8/Bun due to the creation of intermediary objects.
**Action:** Replace `split`, `replace`, and array slicing with direct string indexing (e.g. `charCodeAt()`) and native `String.prototype.slice()` when doing simple parsing tasks to reduce allocation overhead by up to 5x.

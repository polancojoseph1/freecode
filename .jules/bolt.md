...
## 2024-05-20 - String Splitting vs Backward Iteration for Path Manipulation
**Learning:** In path utilities (like `getFilename`, `getDirectory`), using string functions like `.replace()` followed by `.split()` and array slicing generates significant overhead by allocating intermediary strings and arrays. Replacing `.split` with `.lastIndexOf` or a manual backward loop with `charCodeAt` to find separators natively works on the string and avoids heap allocations.
**Action:** When implementing simple string or path manipulation functions that run frequently, avoid creating intermediary arrays (`split`) or regex replace for splitting strings. Prefer index tracking via `lastIndexOf` or manual loops (`charCodeAt`) combined with `slice`.

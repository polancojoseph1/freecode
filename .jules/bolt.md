...
## 2025-03-24 - [SolidJS primitive state proxy overhead in active states]
**Learning:** Using `createStore` for simple active/boolean flags (like tracking `active: true/false` during high-frequency sizing events) adds unnecessary Javascript Proxy overhead where simple primitives would suffice.
**Action:** When migrating or refactoring simple state variables like `active` or `open` booleans inside solid-js components or utilities, refactor `createStore` to `createSignal` to eliminate deep reactivity proxy overhead.

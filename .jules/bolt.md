...
## 2025-03-24 - [Zero-Allocation Array Iteration for UI Rendering]
**Learning:** Using chained higher-order array methods like `.reduce()` or `.filter()`, especially when combined with object spreading (`{ ...acc }`), inside frequently called UI calculation functions (like `estimateSessionContextBreakdown` in `packages/app`) creates unnecessary N+1 intermediate object allocations and functional closures, putting heavy pressure on the garbage collector.
**Action:** For hot-path state calculations during UI rendering loops, manually expand `.reduce()` and `.filter()` into basic `for` loops using localized mutable variables. This ensures zero-allocation iterations and dramatically improves performance.

...
## YYYY-MM-DD - [Concurrent Configuration Merging]
**Learning:** When parallelizing multiple independent async I/O operations (e.g., loading different configuration sources that must be merged in a specific order), simply hoisting the map operations to the top and sequentially awaiting  introduces an unhandled promise rejection risk. If a promise further down the chain rejects faster than the first  resolves, Node.js triggers an `unhandledRejection`.
**Action:** Await all groups concurrently using an outer `await Promise.all([Promise.all(group1), group2, ...])` before sequentially iterating through the resolved results to merge them and preserve priority.
## 2025-03-24 - [Concurrent Configuration Merging]
**Learning:** When parallelizing multiple independent async I/O operations (e.g., loading different configuration sources that must be merged in a specific order), simply hoisting the map operations to the top and sequentially awaiting `Promise.all()` introduces an unhandled promise rejection risk. If a promise further down the chain rejects faster than the first `Promise.all` resolves, Node.js triggers an `unhandledRejection`.
**Action:** Await all groups concurrently using an outer `await Promise.all([Promise.all(group1), group2, ...])` before sequentially iterating through the resolved results to merge them and preserve priority.

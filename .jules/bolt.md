...
## 2025-03-24 - [Parallelize Multiple Network/File Scanning Ops]
**Learning:** Sequential `await` calls inside `for...of` loops (such as mapping over multiple URLs with `Discovery.pull`) cause N+1 synchronous I/O bottlenecks.
**Action:** When multiple independent asynchronous reads or network operations exist, replace the sequential loops with `Promise.all(array.map(...))` to execute the work concurrently and significantly improve initialization or load times.

...
## 2024-05-24 - Path Utility Optimization
**Learning:** For frequently executed path manipulation functions (like `getFilename`, `getDirectory`, `getFileExtension`), using `.split()` allocates intermediate arrays which creates unnecessary garbage collection overhead and significantly reduces execution speed (~2.7x slower). Replacing these with native `.lastIndexOf()` string searches avoids these allocations completely while keeping the code concise and readable. Additionally, complex C-style parsing with manual backward `.charCodeAt()` iteration only provides minimal gains (less than 5-10% improvement) over regex replace for trailing slash removal, sacrificing code readability unnecessarily.
**Action:** Always prefer native string methods (e.g. `.lastIndexOf()`, `.slice()`) over array-allocating methods (e.g. `.split()`) for frequent string and path manipulation utilities where string structures are predictable. Avoid excessive manual character iteration unless dealing with highly specialized processing pipelines, as readability remains paramount.

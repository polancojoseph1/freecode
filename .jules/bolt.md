...
## 2024-05-18 - Replacing chained array methods in hot paths
**Learning:** Chaining array methods like `reduce` and `filter` creates unnecessary intermediate arrays and functional closures. For hot paths (like iterating through lines in patches), replacing these with standard nested `for` loops and localized mutable variables (e.g., checking `charCodeAt(0)` instead of `startsWith`) significantly improves performance by reducing memory allocations and garbage collection overhead.
**Action:** Use nested `for` loops and `charCodeAt(0)` when calculating additions or deletions in diff parsing or similar large-array iterations instead of chaining `reduce` and `filter`.

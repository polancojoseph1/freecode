...
## 2025-03-08 - Path Manipulation Optimization Overhead
**Learning:** In Bun environments, native string pointer arithmetic (like `.lastIndexOf` and `.slice`) is significantly faster than using Regular Expressions (`.replace`, `.split`) and intermediate array allocations for path manipulation functions like `getFilename` and `getDirectory`.
**Action:** When implementing basic path or string parsing, prefer native string matching and `lastIndexOf`/`slice` for hot-path utilities to avoid Garbage Collection and regex compilation overhead, but thoroughly test edge cases.

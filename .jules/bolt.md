...
## 2025-03-24 - [Uint8Array.from Overhead]
**Learning:** Using `Uint8Array.from` with a mapping function (e.g., `(c) => c.charCodeAt(0)`) for base64 decoding string-to-byte conversion in browser environments is significantly slower than using a direct `for` loop to set values on a pre-allocated `Uint8Array`.
**Action:** When converting a string (like from `atob`) to a `Uint8Array`, pre-allocate the array `new Uint8Array(raw.length)` and populate it with a direct `for` loop instead of using `Uint8Array.from`.

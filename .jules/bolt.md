...
## 2024-07-31 - Path Manipulation Utilities Optimization
**Learning:** Native `lastIndexOf` with manual backward iteration for trailing slash stripping in path traversal avoids allocating new arrays and strings compared to `.split()` and regex replacement (`replace()`), running ~3x faster. Replacing regex inside heavily used path utilties (like `getFilename` and `getDirectory`) which are called per file or path during parsing shows measurable results.
**Action:** Favor manual traversal using `.length` and native string operations (`lastIndexOf`, `slice`) for frequently invoked string-manipulation utilities in hot-paths, instead of leveraging regex or chaining high-order array allocations (`split`, `slice`, `join`).

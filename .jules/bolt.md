...
## 2024-07-24 - The Year 2039 ID Collision Trap
**Learning:** Replacing BigInt with standard float64 Number math for timestamp-based ID generation (`currentTimestamp * 0x1000 + counter`) is a critical error. The UNIX timestamp in milliseconds (41 bits) combined with a 12-bit counter perfectly hits 53 bits. In September 2039, the timestamp will cross the 41-bit boundary, causing the value to exceed `Number.MAX_SAFE_INTEGER`. This truncates the lower bits (the counter) and causes massive ID collisions for generated IDs within the same millisecond. BigInt was intentionally used for safety beyond 2039.
**Action:** Never attempt to "optimize" 64-bit ID generation logic or bitwise operations by downcasting BigInt to Number. Leave cryptographic and ID generation logic alone unless specifically targeting documented string allocations.

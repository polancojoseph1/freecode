...
## 2025-03-24 - [Concurrent Async Work inside Loops]
**Learning:** Sequential `await` calls within nested `for...of` loops, like when iterating over directories to scan for files and then reading their contents, introduce significant synchronous bottlenecks. In `theme.tsx`, loading custom themes sequentially caused delays that scale linearly with the number of files and directories.
**Action:** Use `await Promise.all(array.map(...))` to execute independent I/O operations (scanning and reading files) concurrently instead of awaiting each iteration sequentially. When order matters for merging results, map the results into a 2D array matching the original iteration order, and use a final sequential pass to merge the state.

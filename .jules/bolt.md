...
## 2024-07-05 - Optimize diff statistics parsing
**Learning:** In the TUI component for displaying reverted session diffs, calculating additions and deletions using chained array methods (`patch.hunks.reduce` + `hunk.lines.filter`) causes severe performance overhead. This is due to creating intermediate arrays and function closure allocations. For operations parsed frequently (like patch diffs), native char checks inside a single traversal execute significantly faster.
**Action:** Replace higher-order array methods (.reduce, .filter) with standard nested for-loops and `.charCodeAt(0)` checks when tallying diff additions and deletions. Also ensure no unintentional changes to SDK generated files/bun.lock occur by verifying `git status` after dependency installations.

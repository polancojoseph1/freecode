...
## 2024-06-13 - [Path parsing bottlenecks]
**Learning:** In V8/Bun environments, native `String.prototype.split` can sometimes outperform manual string parsing for simple cases (e.g. `path.split(".")`), but for complex string splitting with Regex patterns like `/[\/\\]/` and slice operations, zero-allocation backward iteration with `charCodeAt` is significantly faster (3-4x in benchmarks).
**Action:** When extracting path components or optimizing hot utility paths, replace Regex `.replace` and `.split` calls with `charCodeAt` backward iterations, but benchmark first as native string methods are surprisingly fast for simple single-character splits.

...
## 2025-03-05 - Avoid array methods in performance-critical diff parsing
**Learning:** Hot-path array iterations over `.lines` in diff parsing, especially within the TUI `revertDiffFiles` computation (`packages/opencode/src/cli/cmd/tui/routes/session/index.tsx`), can trigger substantial intermediate allocations and function closure allocations if chained methods like `.reduce()` and `.filter()` are used.
**Action:** Replace functional array chaining with standard nested `for` loops and localized mutable state. Use zero-allocation methods like `.charCodeAt(0)` instead of `.startsWith()` for checking line prefixes ('+' or '-') to avoid creating unneeded sub-strings.

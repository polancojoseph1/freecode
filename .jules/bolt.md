...
## 2024-05-18 - Replacing chained array methods in patch processing
**Learning:** Using chained higher-order array methods like `.reduce()` and `.filter()` in large patch iteration loops causes significant intermediate array creation and function closure allocations.
**Action:** Replace them with a single nested zero-allocation C-style `for` loop and `.charCodeAt(0)` when processing string additions and deletions to maximize performance.

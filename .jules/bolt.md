...
## 2025-07-21 - [Array.reduce Chaining in Diff Summaries]
**Learning:** Chaining multiple `.reduce()` calls on the `diffs` array to calculate `additions` and `deletions` in session summaries causes redundant iterations and function closure allocations on a backend hot-path.
**Action:** Replace chained `.reduce()` calls with a single `for` loop to accumulate multiple values simultaneously, reducing loop overhead and allocations.

...
## 2025-03-08 - TUI Micro-optimizations Rejected
**Learning:** Replacing chained array methods like `.reduce().filter()` with zero-allocation nested `for` loops is a common JS performance trick. However, applying this inside a TUI rendering block (like calculating patch additions/deletions in `packages/opencode/src/cli/cmd/tui/...`) is rejected as a readability-sacrificing micro-optimization, despite benchmarks showing a 10x+ synthetic speedup, because the TUI array sizes are rarely the true system bottleneck.
**Action:** Only apply zero-allocation loop optimizations to proven backend hot-paths or massive data aggregations (e.g., full session summaries), never to UI/TUI rendering logic unless explicitly profiled as a blocking render issue.

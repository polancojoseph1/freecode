...
## 2025-03-24 - [Array.reduce Hot Path Overhead for LRU Caches]
**Learning:** Using chained `.reduce()` methods in critical hot paths like `approxBytes` (which runs frequently to estimate byte size for the `FileContent` LRU cache) introduces significant function allocation overhead, performing ~4x slower on large payloads than nested `for` loops.
**Action:** Always replace `.reduce()` chains with traditional `for` loops in cache byte estimation functions to eliminate intermediate closures.

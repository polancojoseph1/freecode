...
## 2025-03-24 - [Avoid Regex & Array Allocations in Path Parsing]
**Learning:** For frequently executed path string manipulations (`getFilename`, `getDirectory`), utilizing `.replace()` with Regex or `.split()` results in severe overhead from intermediate array and string allocations. In benchmarks, backwards manual string indexing was ~3-4x faster than `.split()`.
**Action:** When working on path utility functions or heavily called string manipulation helpers, optimize by directly indexing strings using loops rather than allocating arrays via `.split()`.

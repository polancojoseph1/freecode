...
## 2025-03-24 - [Concurrent Async Work in Independent Groups]
**Learning:** Even when final configuration values must be merged sequentially in a specific order (e.g. global, custom, project, dir, managed configs), the I/O for reading and parsing these files from the filesystem does not need to be blocked sequentially.
**Action:** Use `Promise.all` to hoist and resolve groups of independent I/O promises concurrently, wait for all groups at once with a nested `Promise.all([Promise.all(group1), Promise.all(group2)])`, and *then* iterate through the awaited results sequentially to preserve correct override and merge precedence.

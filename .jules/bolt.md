...
## 2025-03-24 - [SolidJS Store Proxy Overhead in Hooks]
**Learning:** Using `createStore` to track primitive state (like `filter` strings or `userScrolled` booleans) in SolidJS custom hooks adds unnecessary proxy overhead, similar to UI components.
**Action:** When creating or refactoring SolidJS hooks (e.g. `useFilteredList`, `createAutoScroll`) that track simple primitives, prefer `createSignal` over `createStore` to optimize high-frequency state updates.

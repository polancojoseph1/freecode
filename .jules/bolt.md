...
## 2025-05-28 - [Bounded Concurrency for Subprocesses]
**Learning:** In the `packages/opencode/src/snapshot/index.ts` file, a sequential loop over changed files invoking `git show` using `Process.text` caused an N+1 query problem blocking the async loop, making `Snapshot.diffFull` scale poorly with many modified files. Unbounded use of Promise.all for external commands can result in OS resource limits (like file descriptors limits).
**Action:** Always prefer array mapping and `Promise.all` over sequential await inside `for...of` loops when executing independent external commands like `git show` sequentially. However, when issuing many subprocesses, introduce bounded concurrency via chunking to prevent OS-level crashes.

...
## 2026-04-26 - [Concurrent Async Work inside Configuration Loaders]
**Learning:** In configuration files where deeply nested structures must be merged sequentially, sequentially awaiting the file reading I/O (e.g., `await loadFile(...)`) blocks subsequent independent file reads, causing N+1 latency.
**Action:** Always hoist the instantiation of independent async I/O operations into arrays of promises at the top of the function. Then, iterate through those pre-initialized promises with `await Promise.all(...)` inside `for...of` loops. This allows the network/disk I/O to run concurrently while still enforcing the necessary sequential order of the configuration merges.

...
## 2025-03-09 - Parallelizing Concurrent I/O with Sequential Merging
**Learning:** In codeblocks that process multiple configuration file loading layers independently, assigning `await Promise.all` sequential outputs causes a severe I/O bottleneck. Creating variables for all unresolved promises at the top of the function ensures concurrency.
**Action:** When working with precedence-based config loading, hoist promise creation immediately out of loops and then await them sequentially inside the merging loop to maximize I/O concurrency without breaking deep-merge order.

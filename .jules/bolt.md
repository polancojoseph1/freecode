...
## 2025-03-08 - Optimized array reduction in approxBytes
**Learning:** Using chained `.reduce()` inside hot-path functions (like calculating bytes in file caches or processing large hunks/diffs) creates measurable overhead via intermediate closures, reducing performance.
**Action:** Replace nested array prototype methods with standard nested `for` loops in hot-paths processing large arrays or deep hierarchies.

...
## 2023-10-27 - [Array iterations in Hot Paths]
**Learning:** Chaining array methods like `.reduce()` and `.filter()` in frequently executed code paths (such as parsing large Git patches or tallying UI block diff changes) causes significant overhead due to unnecessary intermediate array creation and per-element function closure allocations.
**Action:** For performance-critical arrays or large data structures, replace chained `.reduce()`/`.filter()` calls with standard `for` loops. When checking string prefixes in these loops, use `.charCodeAt(0)` instead of `.startsWith()` to eliminate additional string allocations and achieve zero-allocation processing.

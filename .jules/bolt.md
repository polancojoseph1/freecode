...
## 2024-12-16 - V8 Optimization for Hex Timestamps
**Learning:** Native `parseInt(hex, 16)` is roughly 3-6x faster than `BigInt("0x" + hex)` parsing and division in V8 environments (like Bun). For the ID format `prefixes[prefix] + "_" + timeBytes.toString("hex") + randomBase62(LENGTH - 12)`, replacing string splitting (`.split("_")[0]`) with `indexOf` and replacing `BigInt` processing with native integer operations when the hex value (48 bits) fits safely within `Number.MAX_SAFE_INTEGER` significantly reduces overhead and avoids unnecessary allocations.
**Action:** When extracting components from strings, prefer non-allocating operations like `indexOf` over `split`. For numeric operations, always use native `Number` methods (like `parseInt` and division with `Math.floor()`) over `BigInt` if the maximum possible value stays under 53 bits (e.g. 12-character hex strings), avoiding unnecessary precision overhead.

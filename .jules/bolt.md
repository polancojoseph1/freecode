...
## 2025-03-24 - [Concurrent Directory Scanning]
**Learning:** Sequential `await` calls inside loops that perform I/O (like reading child directories in `packages/opencode/src/file/index.ts`) introduce severe N+1 blocking bottlenecks during startup or scanning phases.
**Action:** Always map independent I/O tasks to an array of promises and execute them concurrently with `Promise.all` instead of awaiting them inside sequential `for...of` loops.

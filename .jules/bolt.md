...
## 2024-06-14 - BigInt vs Math Shift Performance
**Learning:** In hot path operations like generating Unique IDs where `BigInt` operations or `Buffer` allocation occur repeatedly, Javascript bitwise operators (`>>`, `~`) truncate natively to 32 bits, making direct bitwise logic fail for 48-bit timestamps. However, using simulated bitwise logic via math division (`Math.floor(now / Math.pow(2, 40 - 8*i))`) combined with per-byte two's-complement inversion (`~val & 0xff`) is natively supported up to 53-bits via `Number.MAX_SAFE_INTEGER`, yielding an immediate ~5-10x performance boost by circumventing `BigInt` object overhead and `Buffer`/`Uint8Array` allocations.
**Action:** Always favor native `Number` math division for values beneath `Number.MAX_SAFE_INTEGER` over `BigInt` when decoding or encoding hex chunks, especially inside loops, and apply any necessary two's complement adjustments per chunk instead of to the entire large number.

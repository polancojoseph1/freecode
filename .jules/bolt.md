...
## 2026-07-23 - Zero-Allocation Context Breakdown
**Learning:** Chained `.reduce()` methods combined with object spread operators (`...acc`) within hot-path iterations (like evaluating large arrays of chat context parts) create severe garbage collection pressure by constantly allocating new objects and function closures.
**Action:** Replace functional array iteration methods with localized mutable POJOs and nested `for` loops in hot paths to enable zero-allocation processing and significantly speed up execution.

...
## 2025-04-19 - Concurrent I/O Loading in Config Loaders
**Learning:** Sequential `await Promise.all()` statements for different configuration sources (e.g., global configs, project configs, directory configs) create an N+1 synchronous read bottleneck, delaying the start of subsequent I/O operations until the previous batch finishes.
**Action:** When a specific sequential merge order is required but the underlying read/parse I/O is independent, instantiate all promise structures concurrently at the top of the function. Then, sequentially await the arrays of promises in the exact order needed to preserve overriding logic. This eliminates I/O bottlenecks without altering the deterministic execution order.

...
## 2025-04-25 - [Hoist and Merge Concurrent I/O Arrays]
**Learning:** When a configuration module relies on merging the outputs of multiple I/O promises (like `ConfigPaths.fileInDirectory(...)`) in a strictly sequential order, attempting to wrap everything in a single `.map()` -> `Promise.all()` -> `merge()` chain per array can create hidden N+1 bottlenecks between the arrays themselves.
**Action:** When a function contains multiple sequential `await Promise.all(...)` blocks for independent async I/O operations (e.g., loading different configuration sources that must be merged in a specific order), hoist the instantiation of all promise structures (e.g. `const globalPromises = ...`, `const customPromise = ...`) to the top of the function to allow concurrent execution. Then, execute the sequential `await Promise.all(...)` iterations to preserve the required execution/merge order.

...
## 2025-05-25 - V8 Hex Parsing Performance: BigInt vs parseInt
**Learning:** In V8/Bun, allocating a new `BigInt` from a hex string (e.g., `BigInt("0x" + hex)`) and dividing it incurs massive performance overhead compared to native integer parsing (`parseInt(hex, 16)`). For the monotonic ID generation utility, replacing the BigInt logic and `id.split("_")[0]` with `indexOf` and `parseInt` made ID parsing ~7-10x faster.
**Action:** Always check if a target hex string max value fits within `Number.MAX_SAFE_INTEGER` (9,007,199,254,740,991). A 12-character hex string's max value (281,474,976,710,655) fits comfortably, meaning `BigInt` can be safely avoided for a huge performance win.

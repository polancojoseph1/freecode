...
## 2024-07-31 - [Flaky Asynchronous Test Timeout Optimization]
**Learning:** Certain asynchronous operations initialized inside test blocks (like `tmpdir({ git: true })` or `ShellTool.init()`) can sporadically exceed GitHub Actions runner boundaries, causing a hard CI failure after hitting a default 30s limit.
**Action:** Increase explicit wait timeouts directly on flaky tests (e.g. `test("...", async () => { ... }, 120000)`) rather than injecting artificial `sleep` delays to provide breathing room during heavy CI runner load.

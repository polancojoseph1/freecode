...
## 2024-06-08 - Use parseInt for fast hex timestamp parsing
**Learning:** Native `parseInt(hex, 16)` is much faster than `BigInt("0x" + hex)` in JS. For a 12-character hex string representing a 48-bit number, the maximum value (`0xffffffffffff` = `281,474,976,710,655`) is well within `Number.MAX_SAFE_INTEGER` (`9,007,199,254,740,991`). Therefore, it's safe to use `parseInt` for decoding IDs without precision loss. Additionally, `id.indexOf("_")` + `slice` is faster than `id.split("_")` when extracting the hex portion.
**Action:** Replace `BigInt` parsing with `parseInt` and `split` with `indexOf` + `slice` in high-frequency identifier string parsing methods to improve performance.

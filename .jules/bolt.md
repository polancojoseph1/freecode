...
## 2025-03-24 - [Avoid unnecessary reduce iterations]
**Learning:** Using `Array.reduce` twice to compute `additions` and `deletions` from the same array results in unnecessary iterations over the same collection and creates closure allocations for each element, creating mild performance overhead.
**Action:** When computing multiple aggregates over the same array (e.g. sum of additions and sum of deletions in diff objects), combine them into a single `for` loop to avoid iterating over the array twice and to prevent closure allocations.

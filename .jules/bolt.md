...
## 2025-03-24 - [Array.reduce Hot Path Overhead for Content Cache]
**Learning:** Using `Array.prototype.reduce` for calculating byte sizes on large array payloads (e.g., deeply nested `content.patch?.hunks` iterating over lines) introduces unnecessary per-element function and closure allocation. When processing large items in an often-called path like cache eviction/insertion calculations, this overhead scales poorly compared to conventional loops.
**Action:** Replace `Array.prototype.reduce` with standard `for` loops when aggregating lengths or byte sizes across deeply nested or large arrays in performance-sensitive contexts.

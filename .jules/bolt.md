...
## 2025-06-01 - [Fast Base64 Decoding using Native Buffers]
**Learning:** Native `Buffer.from(value, 'base64').toString('utf-8')` is ~23x faster than browser-compatible `atob` paired with `Uint8Array.from(raw, (x) => x.charCodeAt(0))` for decoding Base64 UTF-8 strings.
**Action:** When performing Base64 string decoding in shared environments, prioritize the native `Buffer` API if available for substantial speedups, and use direct `for`-loop indexing as a fallback for browsers.

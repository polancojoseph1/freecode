...
## 2025-07-08 - String Micro-Optimizations Rejected for Readability
**Learning:** Optimizing `getFilename` and `getDirectory` path manipulation utilities by replacing native regex and `split()` operations with manual backward `charCodeAt()` iteration was rejected. While it improved performance by eliminating array allocations, it transformed three concise lines of code into 30+ lines of dense, C-style string parsing, violating the strict constraint to never sacrifice code readability for micro-optimizations.
**Action:** Do not use manual index iterations or `charCodeAt()` loops for path/string manipulation when it drastically reduces readability, especially when native methods like `lastIndexOf` can provide sufficient performance. Only use nested loops or manual iterations for proven, large-scale hot-path array processing where the readability trade-off is justified by measurable impact.

...
## 2025-03-09 - Native String and Number parsing vs Array/BigInt Allocations
**Learning:** In the Bun/V8 engine, for high-frequency short strings and bounded numbers (like extracting a 48-bit timestamp from a short hex string id), using `String.prototype.split` creates unnecessary array allocations. Using `indexOf` with `slice` prevents array allocations entirely. Furthermore, parsing a 48-bit hex string (which fits entirely within `Number.MAX_SAFE_INTEGER`) using `parseInt(hex, 16)` and applying primitive math operations (like primitive division with `Math.floor()`) is approximately 6-7x faster than coercing strings into `BigInt` objects and doing `BigInt` math.
**Action:** Always prefer native string search methods (`indexOf`, `lastIndexOf`) over `.split()` to avoid intermediate array allocations. When dealing with hex strings or big numbers, if you can mathematically guarantee the result fits safely within `Number.MAX_SAFE_INTEGER`, always parse with `parseInt` and use primitive arithmetic instead of falling back to `BigInt`.

...
## 2025-03-24 - [Avoid Array HOFs for Hot-Path Iterations]
**Learning:** Chaining array higher-order functions like `.reduce()` and `.filter()` alongside string allocation `.startsWith()` creates large, unnecessary intermediate variables inside loops over arrays like diff hunks.
**Action:** When computing sums or analyzing text in a hot-path, use zero-allocation nested `for` loops combined with string indexing methods like `.charCodeAt(0)` to prevent allocating temporary intermediate variables and achieve much higher processing efficiency.

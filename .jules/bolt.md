...
## 2025-03-24 - [Avoid Array.reduce for hot path rendering and iterations]
**Learning:** Chained `Array.prototype.reduce` with object spread (`...acc`) on hot paths like rendering contexts (e.g. `session-context-breakdown.ts`) introduces substantial per-element allocation overhead and intermediate object creation.
**Action:** Replace `reduce` with traditional nested `for` loops and localized mutable variables to eliminate overhead and achieve zero-allocation processing on potentially hot rendering paths.

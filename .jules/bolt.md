...
## 2025-02-12 - Zero-Allocation Path Parsing Edge Cases
**Learning:** Replacing regex/split with `charCodeAt` string iteration in utility functions like `getFilename` and `getDirectory` gives huge performance wins (8-10x) but can introduce subtle behavior changes on absolute edge cases like `path = "/"`. For instance, splitting `/` gives `["", ""]` which historically caused `getDirectory("/")` to return `"/"` but our loop initially returned `""` because it hit the front of the string.
**Action:** When manually parsing strings to replace higher-order array methods, explicitly document and test extreme edge cases (`/`, `///`, `""`, `just-file`) with regression scripts to ensure zero behavioral shifts.

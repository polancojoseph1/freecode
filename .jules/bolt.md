...
## 2025-03-24 - Parallelize Independent I/O Tasks While Maintaining Sequential Merging in configuration Loaders
**Learning:** In deeply nested configurations like those in `tui.ts`, while the merge order of config objects strictly dictates final state, the disk read and parsing I/O can still be parallelized. By returning objects mapping the path/order to the parsed config from an initial `Promise.all` and merging them subsequently in order, N+1 synchronous read blocking was eliminated without breaking precedence rules.
**Action:** When working with sequential dependency resolution patterns where order matters, map the I/O into a `Promise.all` array and defer the state mutation (like deep merging) into a second sequential loop over the awaited results.

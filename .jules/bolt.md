...
## 2025-03-24 - [Avoid Array.reduce combined with filter for nested string iteration]
**Learning:** Using `Array.prototype.reduce` and `Array.prototype.filter` to count additions/deletions in large parsed git patches introduces severe closure and intermediate array allocation overhead.
**Action:** When counting strings based on prefix conditions (like starting with `+` or `-`) within nested arrays, use a standard nested `for` loop combined with `.charCodeAt(0)` to process lines with zero-allocations.

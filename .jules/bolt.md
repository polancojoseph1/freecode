...
## 2024-03-05 - Optimizing 12-char hex string parsing in Node/Bun
**Learning:** For frequently executed string manipulations like timestamp parsing from IDs, native `indexOf` combined with `parseInt(hex, 16)` is significantly faster than using `.split()` and `BigInt` parsing/division when the hex string is guaranteed to be within the safe integer limit (e.g. 12-char hex fits in Number.MAX_SAFE_INTEGER). Also, standard JavaScript division yields floating-point results, so `Math.trunc()` must be used to replicate BigInt's integer truncation.
**Action:** Always prefer native string methods and `Number` parsing with Math.trunc() for safe integer bounds over array allocations via `.split()` and slow `BigInt` operations in hot paths.

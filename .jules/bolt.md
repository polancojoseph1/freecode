...
## 2024-06-04 - Optimize Hex to Number Conversion and String Parsing in V8
**Learning:** Native `parseInt(hex, 16)` is significantly faster (~18x) than parsing and dividing `BigInt` values for hex conversions in V8/Bun environments. If the maximum possible value of a hex string fits safely within `Number.MAX_SAFE_INTEGER` (e.g., a 12-character hex string which is 48 bits, well below the 53-bit limit), using `parseInt` is a major performance boost. Additionally, using `.indexOf()` and `.slice()` instead of `.split()` prevents intermediate array allocations, improving speed (~6x overall improvement in our timestamp extraction benchmark).
**Action:** Always verify the domain range of parsed numbers. If they fit in `Number.MAX_SAFE_INTEGER` (up to 9,007,199,254,740,991 or ~13 hex chars), use native `parseInt()` over `BigInt()`. Avoid `.split()` when extracting simple prefixes from strings if a single index locator works.

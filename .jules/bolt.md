...
## 2025-03-24 - [Avoid Regex and Splitting in Hot-Path String Utilities]
**Learning:** Using regex (e.g. `.replace(/[\/\\]+$/, "")`) or `.split()` for simple path manipulations generates significant overhead due to object allocations and regex engine invocation. In high-throughput contexts like directory or filename resolution, native string methods (`.lastIndexOf()`, `.slice()`, and explicit character loop comparisons) are twice as fast and bypass intermediate array allocations completely.
**Action:** When implementing or optimizing hot-path utility functions (like `getFilename` or `getDirectory`), prefer direct character indexing (`path[i]`) and `lastIndexOf` over `split` and `replace`.

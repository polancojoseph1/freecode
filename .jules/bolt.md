...
## 2024-05-18 - [Parallelize Independent Awaited Operations]
**Learning:** Sequential `await` calls inside loops or map functions that can run independently (like batch DB operations and async cascading deletes) create significant N+1 performance bottlenecks.
**Action:** Identify sequential loops over async operations (e.g. `for (const x of arr) { await fn(x) }`) and replace them with `await Promise.all(arr.map(x => fn(x)))` when the operations don't strictly require serial execution.

...
## 2025-06-02 - Optimize string manipulation in Path Utilities
**Learning:** Native `String.prototype.split()` and regex operations (`replace`) are significantly slower for basic path parsing than manual zero-allocation backward `charCodeAt` iteration. When avoiding arrays and intermediary objects, execution is up to 2-4x faster.
**Action:** When working on heavily used path manipulation or simple string extraction utilities, prioritize manual backward iteration with `charCodeAt` and native `slice` instead of regex and `split` to avoid memory allocation bottlenecks and improve performance.

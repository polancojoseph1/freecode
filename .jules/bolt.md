...
## 2025-03-24 - [Unintended modifications to bun.lock and SDK types]
**Learning:** Running `bun install` locally can sometimes inadvertently update `bun.lock` and trigger automatic SDK generation scripts, leading to unintended and out-of-scope modifications (like breaking error handling changes in `client.gen.ts`).
**Action:** Always carefully run `git status` and `git diff --cached` after running commands like `bun install`. Ensure only the intentionally modified files are staged and committed, and use `git restore` and `git checkout` to discard accidental file changes before finalizing a PR.

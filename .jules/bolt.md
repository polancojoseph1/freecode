...
## 2025-04-12 - [Array.reduce Hot Path Overhead in Session Context Breakdown]
**Learning:** Using `Array.prototype.reduce` for calculating the session context breakdown introduces unnecessary function allocation overhead compared to basic `for` loops, especially when the message list and their parts list is very large.
**Action:** Replaced `.reduce` with traditional `for` loops in hot path iterations like `estimateSessionContextBreakdown` to remove per-element closures, dramatically increasing execution performance.

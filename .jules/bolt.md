...
## 2025-03-24 - [BigInt allocation overhead in timestamp extraction]
**Learning:** Parsing 12-character hex strings into `BigInt` and performing `BigInt` division adds significant overhead (about 7x slower) in hot paths.
**Action:** Since a 12-character hex string (48 bits) easily fits within JavaScript's `Number.MAX_SAFE_INTEGER` (53 bits), prefer `parseInt(hex, 16)` and `Math.floor(value / div)` over `BigInt` parsing and division to improve performance in timestamp extraction logic.
